...
CREATE OR REPLACE PACKAGE BODY DB_COMERCIAL.CMKG_PROMOCIONES_UTIL_REG AS
  ...
  WHEN OTHERS THEN
    ...
    IF Lv_Observacion IS NULL THEN
      IF Pv_CodigoGrupoPromocion = 'PROM_MENS' THEN
        Lv_Observacion := 'No otorg� Promoci�n por descuento Mensual, revisar en info_error.';
      ELSE
        Lv_Observacion := 'No otorg� Promoci�n por descuento de Instalaci�n, revisar en info_error.';
      END IF;
      Pv_Observacion := Lv_Observacion;
    END IF;
