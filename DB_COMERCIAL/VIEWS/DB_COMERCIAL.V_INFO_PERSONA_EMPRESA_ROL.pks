/* Formatted on 5/2/2023 9:55:59 AM (QP5 v5.336) */
CREATE OR REPLACE FORCE VIEW DB_COMERCIAL.V_INFO_PERSONA_EMPRESA_ROL
(
    PER_ID_PERSONA,
    PER_NOMBRES,
    PER_IDENTIFICACION_CLIENTE,
    IEG_COD_EMPRESA,
    ID_PERSONA_ROL,
    PERSONA_ID,
    EMPRESA_ROL_ID,
...
BEQUEATH DEFINER
AS
    SELECT IP.ID_PERSONA                 AS PER_ID_PERSONA,
           ...
           IPER."PERSONA_EMPRESA_ROL_ID_TTCO",
           IPER."REPORTA_PERSONA_EMPRESA_ROL_ID",
           IPER."ES_PREPAGO",
           IPER."USR_ULT_MOD",
           IPER."FE_ULT_MOD"
      FROM DB_COMERCIAL.INFO_PERSONA_EMPRESA_ROL  IPER
           ...
           INNER JOIN DB_COMERCIAL.INFO_EMPRESA_GRUPO IEG
               ON IER.EMPRESA_COD = IEG.COD_EMPRESA
     WHERE     IPER.ESTADO NOT IN
                   (SELECT IPD.VALOR1
                      FROM DB_GENERAL.ADMI_PARAMETRO_CAB  IPC,
                           DB_GENERAL.ADMI_PARAMETRO_DET  IPD
                     WHERE     IPD.PARAMETRO_ID = IPC.ID_PARAMETRO
                           AND IPD.ESTADO = 'Activo'
                           AND IPC.ESTADO = 'Activo'
                           AND IPC.NOMBRE_PARAMETRO =
                               'ESTADOS_PERSONA_EMPRESA_ROL')
           AND IPER.EMPRESA_ROL_ID IN
                   ((SELECT ID_EMPRESA_ROL
                       FROM DB_COMERCIAL.INFO_EMPRESA_ROL  IERM,
                            DB_GENERAL.ADMI_ROL            AR,
                            DB_GENERAL.ADMI_TIPO_ROL       ATR
                      WHERE     AR.ID_ROL = IERM.ROL_ID
                            AND ATR.ID_TIPO_ROL = AR.TIPO_ROL_ID
                            AND ATR.DESCRIPCION_TIPO_ROL IN
                                    (SELECT IPD.VALOR1
                                       FROM DB_GENERAL.ADMI_PARAMETRO_CAB IPC,
                                            DB_GENERAL.ADMI_PARAMETRO_DET IPD
                                      WHERE     IPD.PARAMETRO_ID =
                                                IPC.ID_PARAMETRO
                                            AND IPD.ESTADO = 'Activo'
                                            AND IPC.ESTADO = 'Activo'
                                            AND IPC.NOMBRE_PARAMETRO =
