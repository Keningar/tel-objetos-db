 ...
 * Creacion de data para nuevo Canal de pago Banco Produbanco Ecuanet
...
DECLARE
    ...
    CURSOR c_getdata IS
        select a.* from DB_BUSPAGOS.info_config_ent_rec_emp a where a.entidad_rec_empresa_id = (select d.ID_ENTIDAD_REC_EMPRESA from DB_BUSPAGOS.info_entidad_rec_empresa d where d.USUARIO_BUS = 'produbanco');
...
BEGIN
    ...
    'jihidalgo',
    null,
    (select ID_ENTIDAD_RECAUDADORA from DB_BUSPAGOS.admi_entidad_recaudadora where codigo = 'bcoprodubancoen'),
    (select ID_EMPRESA from DB_BUSPAGOS.admi_empresa where codigo = 'ecuanet'),
    'bcoprodubancoen',
    'jU+d6VLCrl4V2YVrxCWmLA==',
    'bcoprodubancoen',
    'jU+d6VLCrl4V2YVrxCWmLA==',
    null,
    null,
    ...
    LOOP
        ...
        INSERT INTO DB_BUSPAGOS.INFO_CONFIG_ENT_REC_EMP VALUES (
            ...
            'jihidalgo',
            NULL,
            (select n.id_entidad_rec_empresa from db_buspagos.info_entidad_rec_empresa n where n.usuario_bus = 'bcoprodubancoen'),
    ...
    WHERE PARAMETRO = 'MAIL_NOTIFICATION_BODY' 
    AND ENTIDAD_REC_EMPRESA_ID = (select d.ID_ENTIDAD_REC_EMPRESA from DB_BUSPAGOS.info_entidad_rec_empresa d where d.USUARIO_BUS = 'bcoprodubancoen');
