CREATE EDITIONABLE PACKAGE                                       INKG_SINC_ARCGIS AS 
  ...
  */
  ...
  Pv_Mensaje      IN OUT VARCHAR2);
    ...
    /**
    * Documentacion para   PROCEDURE P_INACTIVAR_UM
    *
    ...
    * @param Ln_existeLogin          NUMBER(5)  Retorna el resultado consultado
    * @param Lv_FechaCancela         VARCHAR(60)  Fecha que se realiza el update
    * @param Lv_EstadoNuevo          VARCHAR(60)  Estado para actualizar en la tabla FIBER_TN 
    *
    * @author Liseth Candelario <lcandelario@telconet.ec>
    * @version 1.0 12-07-2022
    *
    */
  PROCEDURE P_INACTIVAR_UM(
      Pv_UsrCancelacion IN VARCHAR2,
      Pv_NombreSwitch   IN VARCHAR2,
      Pv_PuertoSwitch   IN VARCHAR2,
      Pv_LoginPunto     IN VARCHAR2,
      Pv_Prefijo        IN VARCHAR2,
...
CREATE EDITIONABLE PACKAGE BODY                                                                                                                                      INKG_SINC_ARCGIS AS
...
END P_INSERTAR_UBICACION;
 ...
 PROCEDURE P_INACTIVAR_UM(
      ...
      Pv_Mensaje OUT VARCHAR2 )
    AS
      Ln_existeLogin  NUMBER(5);
      Lv_FechaCancela VARCHAR2(60) := TO_CHAR(sysdate, 'YYYY-MM-DD HH:MI:SS');
      Lv_EstadoNuevo  VARCHAR2(60) := 'Cancel';
    BEGIN
      --
      -- Inicio de si existe el login
      IF Pv_Prefijo = 'MD' THEN
        -- Inicio de Si es MD
        SELECT COUNT(OBJECTID)
        INTO Ln_existeLogin
        FROM SDE.FIBER_MD@dblink_arc
        WHERE LOGIN = Pv_LoginPunto;
        -- consulta si hay un registro con ese login
        IF Ln_existeLogin >0 THEN
          UPDATE SDE.FIBER_MD@dblink_arc
          SET USUARIO_CANCELA =Pv_UsrCancelacion,
            FECHA_CANCELACION =Lv_FechaCancela,
            ESTADO_SERVICIO   =Lv_EstadoNuevo
          WHERE OBJECTID     IN
      ...
      ELSE
        ...
        SELECT COUNT(OBJECTID)
        INTO Ln_existeLogin
        FROM SDE.FIBER_TN@dblink_arc
        WHERE NOMBRE_SWITCH = Pv_NombreSwitch
        AND PUERTO_SWITCH   = Pv_PuertoSwitch;
        -- consulta si hay un registro con ese login
        IF Ln_existeLogin >0 THEN
          UPDATE SDE.FIBER_TN@dblink_arc
          SET USUARIO_CANCELA =Pv_UsrCancelacion,
            FECHA_CANCELACION =Lv_FechaCancela,
            ESTADO_SERVICIO   =Lv_EstadoNuevo
          WHERE OBJECTID     IN
    ...
    WHEN OTHERS THEN
      Pv_Status  := 'ERROR';
      Pv_Mensaje := substr('ERROR: '||SQLERRM,0,200);
  END P_INACTIVAR_UM;
