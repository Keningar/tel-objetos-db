CREATE    OR REPLACE VIEW "NAF47_TNET"."V_SOLICITUD_COMPRA" ("EMPRESA_ID", "ID_SOLICITUD", "ID_SOLICITUD_DETALLE", "ID_ARTICULO", "DESCRIPCION_ARTICULO", "CANTIDAD", "FECHA", "USR_CREACION", "ESTADO", "REGION", "ID_ORDEN_COMPRA", "USR_ORDEN_COMPRA", "FECHA_ORDEN_COMPRA", "ESTADO_ORDEN_COMPRA", "ORDEN_COMPRA_NAF", "PROVEEDOR_COMPRA", "ESTADO_COMPRA") AS 
  WITH
  V_SOLICITUD AS
    (SELECT VS.EMPRESA_ID,
     ...
     JOIN DB_COMPRAS.INFO_SOLICITUD VS ON VS.ID_SOLICITUD = VSD.SOLICITUD_ID),
  ...
  V_ORDEN_COMPRA AS
    (SELECT A.EMPRESA_ID,
     ...
     JOIN DB_COMPRAS.INFO_ORDEN_COMPRA_DETALLE B ON B.ORDEN_COMPRA_ID = A.ID_ORDEN_COMPRA
...
SELECT E.CODIGO AS EMPRESA_ID,
  ...
  (SELECT  DECODE(TA.ESTADO,'P', 'Pendiente', 
                            'E', 'Emitido', 
                            'F', 'Finalizado', 
                            'I', 'Incompleto', 
                            'X', 'Anulado', 
...
FROM V_SOLICITUD S
  ...
  JOIN NAF47_TNET.ARINDA AR ON AR.NO_ARTI = S.ARTICULO_ID AND AR.NO_CIA = E.CODIGO
  LEFT JOIN V_ORDEN_COMPRA IOC ON IOC.ID_ORDEN_COMPRA = S.ORDEN_COMPRA_ID AND IOC.ARTICULO_ID = S.ARTICULO_ID--IOC.EMPRESA_ID = S.EMPRESA_ID AND IOC.PEDIDO_ID = S.PEDIDO_ID 
