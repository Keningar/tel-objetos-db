create or replace FUNCTION            CCSALDO_FINAL_SUBCLI_PEND_DIV(PV_NOCIA          VARCHAR2,
                                                          PV_GRUPO          VARCHAR2,
                                                          PV_CLIENTE        VARCHAR2,
 ...
 CURSOR C_SALDOSDEBITO_CREDITOS(CV_NOCIA          VARCHAR2,
  ...
  SELECT NVL(SUM(ta.DEBITOS),0) DEBITOS, NVL(SUM(ta.CREDITOS),0) CREDITOS
   FROM v_documentos_estado_cta_DIV ta
   WHERE ta.ESTADO <> 'P'
   ...
   --
   AND (PV_MUESTRA = 'Todos los documentos' OR ta.saldo != 0)
   --
...
END CCSALDO_FINAL_SUBCLI_PEND_DIV;