CREATE OR REPLACE PACKAGE            INKG_TRANSACCIONES_PEDIDOS AS
  ...
  /**
  * Documentacion para PROCEDURE P_INSERT_INFO_PEDIDO
  * Paquete que inserta la cabecera de un pedido
  * @author Veronica Moreno <vmmoreno@telconet.ec>
  ...
  */
  ...
  /**
  * Documentacion para PROCEDURE P_INSERT_INFO_PEDIDO_DETALLE
  ...
  * @version 1.0 05-07-2021
   ...
   @param Pd_FeUltMod                    IN DATE fecha ultima modificacion
   @param Pv_UsrCreacion                 IN VARCHAR2 usuario creacion
   @param Pv_UsrUltMod                   IN VARCHAR2 usuario ultima modificacion
   @param Pv_IpCreacion                  IN VARCHAR2 ip creacion
   ...
   @param Pv_UsrAprobacionId             IN VARCHAR2 usuario aprobacion id
   @param Pv_UsrAprobacion               IN VARCHAR2 usuario aprobacion
   @param Pv_EsAprobacion                IN VARCHAR2 es aprobacion
   @param Pn_PedidoAprobacionId          IN NUMBER pedido aprobacion id
  ...
  */
  ...
  /* Documentacion para funcion F_GET_ADMI_EMPRESA
  * Funcion que retorna un registro, buscado por su codigo y estado
  * @author Veronica Moreno <vmmoreno@telconet.ec>
  * @version 1.0 05/07/2021
  
  @param Pv_Codigo   codigo de empresa
  ...
  */
  ...
  /**
  * Documentacion para funcion F_GET_ADMI_DEPARTAMENTO
  * Funcion que retorna un registro del codifo del departamento, buscado por empresa, codigo y estado
  * @author Veronica Moreno <vmmoreno@telconet.ec>
  * @version 1.0 05/07/2021
  
  @param Pv_NoCia compania
  @param Pv_Codigo codigo de departamento
  @param Pv_Estado estado
  ...
  */
  ...
  /**
  * Documentacion para funcion F_GET_USUARIO_NAF
  * Funcion que retorna  los datos de un usuario, buscado por empresa, codigo y estado
  * @author Veronica Moreno <vmmoreno@telconet.ec>
  * @version 1.0 05/07/2021
  
    @param Pv_NoCia  compania
    @param Pv_NoEmple codigo de empleado
    @param Pv_Estado estado
  */
  ...
  /**
  * Documentacion para funcion F_GET_ADMI_PARAMETRO
  * Funcion que retorna un registro de ADMI_PARAMETRO, buscado por empresa, codigo y estado
  * @author Veronica Moreno <vmmoreno@telconet.ec>
  * @version 1.0 05/07/2021
  
   @param Pv_EmpresaId  compania
   @param Pv_Codigo codigo de parametro
   @param Pv_Estado estado
  */
  ...
  /**
  * Documentacion para funcion P_PEDIDO_MASIVO_INST
  * Procedimiento que realiza el insert masivo diario de pedidos y detalle de pedidos
  * de las tareas realizadas de instalaciones de ultima milla de cada uno de los tecnicos en Megadatos
  * buscado por empresa, tipo_ejecucion.
  * @author Veronica Moreno <vmmoreno@telconet.ec>
  * @version 1.0 05/07/2021
  *
  * @author Veronica Moreno <vmmoreno@telconet.ec>
  * @version 1.1 06/02/2022 - Se modifica proceso para mejorar busquedas de data Jefe Cuadrilla, Tenicos, Pedidos generados existentes
  *                           articulos asignados a tenicos en control custodios. Tambien se mejora control de errores por registros tareas,
  *                           registros plantillas y por envio de correo electronico. Tambien se corrige generacionmas detallada de los errores
  *                           en log-errores-pedidos-masivos.
  *                            Se agregan parametros de Fecha-Hora Inicio y Fecha-Hora Fin enviadas por proceso que lee configuracion 
  *                           para ejecucion de JOB
  *
  ...
  * @param Pv_Empresa      IN VARCHAR2     recibe codigo de compania
  ...
  */
  ...
  /**
  * Documentacion para PROCEDURE P_EJECUTAR_JOB
  * Procedimiento que ejecuta 2 tareas progamadas para la ejecucion masiva de creacion de pedidos.
  * @author Veronica Moreno <vmmoreno@telconet.ec>
  * @version 1.0 04-08-2021
  *
  * @author Luis Lindao <llindao@telconet.ec>
  * @version 1.1 06/02/2022 - Se modifica para procesar JOB mediante condiguracion en tabla de parametros
  ...
  */
  ...
  /**
  * Documentacion para P_LOG_ERROR_PEDIDOS_AUT
  * Procedimiento que inserta los lideres que no se encuentran configurado en NAF
  * @author Veronica Moreno <vmmoreno@telconet.ec>
...
CREATE OR REPLACE PACKAGE BODY            INKG_TRANSACCIONES_PEDIDOS AS
  --
  ...
  PROCEDURE P_PEDIDO_MASIVO_INST ( Pd_FechaInicio  IN DATE,
    ...
    PEDIDO_AUTOMATICO              CONSTANT VARCHAR2(17) := 'Pedido Automatico';
    ...
    --Correo remitente para el envio de notificacion
  ...
  BEGIN
    ...
    --
    -- cursor que recupera tareas de instalaciones generadas a los tecnicos 
    FOR Lr_ConsultaTarea IN C_CONSULTA_TAREA_TELCOS  LOOP
      ...
      BEGIN
        ...
        END IF;
        --
        -- se verifica configuracion de jefe cuadrilla en pedidos de Instalaciones (Bodega-Solicitante)
        ...
        END IF;
        --
        -- se recupera datos de Tenicos en NAF
        IF C_DATOS_EMPLEADO_NAF%ISOPEN THEN
          CLOSE C_DATOS_EMPLEADO_NAF;
        END IF;
        ...
        IF Lr_DatosNafTecnico.No_Emple IS NULL THEN
          Pv_MensajeError := 'No se pudo recuperar datos de empleado en NAF para Tecnico '||Lr_ConsultaTarea.Login_Tecnico||' empresa '||Lr_ConsultaTarea.Empleado_Empresa_Id;
          Raise Le_ErrorTarea;
        END IF;
        --
        ...
        IF Lr_DatosCustodio.Depto_Instalacion_Id IS NULL THEN
          Pv_MensajeError := 'Tecnico '||Lr_ConsultaTarea.Login_Tecnico||' se encuentra asignado al departamento '||Lr_DatosCustodio.Departamento_Id||', el cual no esta configurado como departamento de instalacion.';
          Raise Le_ErrorTarea;
        END IF;
        --
        -- se recuperan parametros para envio de correo electronico
        ...
        Lr_DataUsuarioJefe := INKG_TRANSACCIONES_PEDIDOS.F_GET_USUARIO_NAF(Lr_ConsultaTarea.Empleado_Empresa_Id,-- Lr_Empresa.Codigo,
                                                                           ...
                                                                           Pv_MensajeError);
        --
        -- cursor que recupera datos de las plantillas de articulos
        FOR Lr_DatosPlantilla IN C_DatosPlantilla LOOP
          ...
          BEGIN  
            ...
            -- si existe configuracion articulo-Depto se valida que este cofigurado el departamento al que pertenece el tecnico
            ...
            IF Lr_DatosPedidoMasivo.ID_PEDIDO IS NOT NULL THEN
              Pv_MensajeError := 'El Tecnico '|| Lr_ConsultaTarea.Login_Tecnico ||' tiene pedido en curso ' ||Lr_DatosPedidoMasivo.ID_PEDIDO||' para el articulo '||Lr_DatosPlantilla.Material_Cod;
              Raise Le_ErrorPlantilla;
            END IF;
            --
            

            -- se recupera la cantidad de articulos asignados al tecnico para validar las cantidades con plantilla
            ...
            --
            -- se realiza la conversion pues los pedidos se generan en base unidades de inventarios
            ...
            --
            ------------------------------------------------------------------------
            -- Si cabecera de pedido no ha sdo generado aun, se procede a generar --
            ------------------------------------------------------------------------
            IF (Lr_ControlCustodio.No_Articulo IS NULL OR 
              ...
              Lr_InfoPedido.Observacion := 'Pedido Automatico';
              ...
              --
              Lr_InfoPedido.Id_Pedido := DB_COMPRAS.MIG_SECUENCIA.SEQ_INFO_PEDIDO;
            ...
            Lr_Empresa := INKG_TRANSACCIONES_PEDIDOS.F_GET_ADMI_EMPRESA( Lr_DatosPlantilla.Empresa_Cod, --Lr_DatosLider.No_Cia,
                                                                         NAF47_TNET.GEK_VAR.Gr_Estado.ACTIVO);-- Lv_EstadoActivo);
            --
            --Si Lider de cuadrilla no tiene asignado el codigo de material genera un pedido
            IF (Lr_ControlCustodio.No_Articulo IS NULL OR 
                Lr_ControlCustodio.CANTIDAD <= Lr_DatosPlantilla.VALOR_MINIMO) THEN
              --
              INKG_TRANSACCIONES_PEDIDOS.P_INSERT_INFO_PEDIDO_DETALLE(DB_COMPRAS.MIG_SECUENCIA.SEQ_INFO_PEDIDO_DETALLE,
    ...
    COMMIT;
    --
    -- si se genero algun pedido se procede a enviar notificaciones de correo.  
        ...
        EXCEPTION
          WHEN OTHERS THEN
            DB_GENERAL.GNRLPCK_UTIL.INSERT_ERROR('NAF',
                                                 ...
                                                 'Error en proceso envio correos notificacion. '||SQLERRM || ' - ' ||DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
                                                 --GEK_CONSULTA.F_RECUPERA_LOGIN,              emunoz 11012023
                                                 LOWER(USER),                                         --emunoz 11012023
                                                 SYSDATE,
                                                 GEK_CONSULTA.F_RECUPERA_IP);
  ...
  BEGIN
    INSERT INTO NAF47_TNET.LOG_ERROR_PEDIDOS_AUT
      (ID_LOG_PEDIDO,
       ...
       ID_PERSONA_ROL)
    VALUES
      (NAF47_TNET.MIG_SECUENCIA.SEQ_LOG_PEDIDO_AUT,
