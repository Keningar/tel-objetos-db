CREATE OR REPLACE package            INKG_TRANSFERENCIAS is
  ...
  *
  * @param Pv_NoCia          IN VARCHAR2     Codigo que identifica a la empresa de la transferencia
  * @param Pv_ObsInfoDetalle IN VARCHAR2     Detalle de las transferencias pendientes por bodegas
  * @param Pv_Login          IN VARCHAR2     Login sobre el cual se creara la tarea.
  ...
  *
  * @author Luis Lindao <llindao@telconet.ec>
  * @version 1.1 01/03/2022 - Se agrega validacion detalle articulos procesados.
  *
  * @param Pv_NoDocu       IN VARCHAR2     Numero de identificacion de Transferencia
  * @param Pv_Centro       IN VARCHAR2     Codigo centro distribucion a la que pertenece la transferencia
  * @param Pv_NoCia        IN VARCHAR2     Codigo que identifica a la empresa de la transferencia
  * @param Pv_MensajeError IN OUT VARCHAR2 Retorna mensaje de error
  */
  ...
  *
  * @param Pv_NoDespacho    IN VARCHAR2          Numero de identificacion de despacho bodega de salida
  * @param Pv_Centro        IN VARCHAR2          Codigo centro distribucion a la que pertenece la transferencia
  * @param Pt_DetalleTransf IN Gt_Detalle_Transf Detalle de articulos a procesar.
  * @param Pv_NoCia         IN VARCHAR2          Codigo que identifica a la empresa de la transferencia
  * @param Pv_MensajeError  IN OUT VARCHAR2      Retorna mensaje de error
  */
...
CREATE OR REPLACE package body            INKG_TRANSFERENCIAS is
  ...
  PROCEDURE P_CREA_TAREA_TRANSFERENCIAS ( Pv_NoCia              IN VARCHAR2,
    ...
    Lv_Asunto             Varchar2(150):= 'Nueva Tarea, Transferencias pendientes recepcion - Actividad #';
  ...
  BEGIN 
    ...
    CLOSE C_TAREA;
    --
    Ln_IdDetalle:= DB_SOPORTE.MIG_SECUENCIA.SEQ_INFO_DETALLE;
    --
    ...
    --
    Ln_IdCriterioAfectado:= DB_SOPORTE.MIG_SECUENCIA.SEQ_INFO_CRITERIO_AFECTADO;
    --
    ...
    VALUES 
         ( Ln_IdCriterioAfectado,
           ...
           GEK_CONSULTA.F_RECUPERA_IP);
     --
     Ln_IdParteAfectada:= DB_SOPORTE.MIG_SECUENCIA.SEQ_INFO_PARTE_AFECTADA;
     --
    ...
    INSERT INTO DB_SOPORTE.INFO_DETALLE_ASIGNACION 
         ( ID_DETALLE_ASIGNACION,
           ...
           DEPARTAMENTO_ID)
    VALUES
         ( DB_SOPORTE.MIG_SECUENCIA.SEQ_INFO_DETALLE_ASIGNACION,
           Ln_IdDetalle,
    ...
    INSERT INTO DB_SOPORTE.INFO_TAREA_SEGUIMIENTO 
         ( ID_SEGUIMIENTO,
           ...
           PERSONA_EMPRESA_ROL_ID)
    VALUES 
         ( DB_SOPORTE.MIG_SECUENCIA.SEQ_INFO_TAREA_SEGUIMIENTO, 
           Ln_IdDetalle,
    ...
    INSERT INTO DB_SOPORTE.INFO_DETALLE_HISTORIAL 
         ( ID_DETALLE_HISTORIAL, 
           ...
           ACCION)
    VALUES
         ( DB_SOPORTE.MIG_SECUENCIA.SEQ_INFO_DETALLE_HISTORIAL,
           Ln_IdDetalle,
           'Tarea fue Asignada a '||Lr_Departamento.Nombre_Departamento,
           Lv_Asignada,
           Pv_Login, 
    ...
    CLOSE C_CLASE_DOC;
    --
    Ln_IdDocumento:= DB_COMUNICACION.MIG_SECUENCIA.SEQ_INFO_DOCUMENTO;
    ...
    --
    Ln_IdComunicacion:= DB_COMUNICACION.MIG_SECUENCIA.SEQ_INFO_COMUNICACION;
    ...
    VALUES 
         ( DB_COMUNICACION.MIG_SECUENCIA.SEQ_DOCUMENTO_COMUNICACION,
           Ln_IdDocumento,
           Ln_IdComunicacion,
           ESTADO_ACTIVO,
           Pv_Login,
 ...
 EXCEPTION 
      ...
      FOR Lr_Horas IN C_VALIDAR_HORAS LOOP
        ...
        IF Lr_JefeBodega.Login_Empleado IS NULL THEN
          ...
          DB_GENERAL.GNRLPCK_UTIL.INSERT_ERROR('NAF',
                                               ...
                                               'No se ha definido Jefe Regional de Bodega para empresa: '||Lr_Horas.No_Cia||', region: '||Lr_Region.No_Region||'. No es posible generar notificacion.',
                                               --GEK_CONSULTA.F_RECUPERA_LOGIN,           emunoz 12012023
                                               LOWER(USER),                                      --emunoz 12012023
                                               SYSDATE,
                                               GEK_CONSULTA.F_RECUPERA_IP);
  ...
  BEGIN
    ...
    --
    -- A?adir proceso por Transferencia de Consignacion
    ...
    FOR Lr_DetalleDoc IN C_DETALLE_DOCUMENTO ( Lr_Arinte.No_Docu,
      ...
      IF Gr_ArticuloBodeg.Costo_Uni IS NULL THEN
        Pv_MensajeError := 'El articulo '||Lr_DetalleDoc.No_Arti||' no existe en la bodega ORIGEN: '||Lr_Arinte.Bod_Orig;
        Raise Le_Error;
      END IF;
      --
      --
      ...
      ELSE
        IF Lr_DetalleDoc.Metodo_Costo = 'E' AND NVL(Lr_DetalleDoc.Costo_Estandar,0) = 0 THEN
          Pv_MensajeError := 'El articulo '|| Lr_DetalleDoc.No_Arti ||' no tiene definido costo estandar';
          Raise Le_Error;
        END IF;
        --
    ...
    IF NVL(Gr_Arinml.linea,0) = 0 THEN
      Pv_mensajeError := 'Existe inconsistencia de informacion entre cabecera y detalle del documento, favor eliminar el detalle articulos y vuelva a ingresar.';
      Raise Le_Error;
    END IF;
    --
    --
  ...
  BEGIN
    ...
    IF Lr_DatosDespacho.No_Cia IS NULL THEN
      Pv_mensajeError := 'No se encontro trasnferencia de salida '||Pv_NoDespacho||', favor revisar ';
      RAISE Le_Error;
    END IF;
    --
    -- se recuperan parametros para asignar a documentos inventarios.
    ...
    FOR Li_Indice IN 1..Pt_DetalleTransf.LAST LOOP
      ...
      ELSE
        IF Lr_DatosArticulo.Metodo_Costo = 'E' AND NVL(Lr_DatosArticulo.Costo_Estandar,0) = 0 THEN
          Pv_MensajeError := 'El articulo '|| Gr_Arinml.No_Arti ||' no tiene definido costo estandar';
          Raise Le_Error;
        END IF;
        --
      ...
      --
      -- Se asigna documento traslado entrada en pre ingreso, temporalemte se asigno con documento despacho
