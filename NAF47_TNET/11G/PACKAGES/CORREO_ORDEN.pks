CREATE OR REPLACE PACKAGE NAF47_TNET.CORREO_ORDEN AS
  ...
  * Variable Registro que permite pasar por parametro los datos necesarios para notificacion de orden compra autorizada.
  ...
  * @version 1.4 15/01/2023 Se procede al cambio de GEK_CONSULTA.F_RECUPERA_LOGIN por la sentencia LOWER(USER)  
  ...
  * Procedure que crea correo electronico para notificar a usuarios las autorizaciones de ordenes de compra.
  * @author llindao <llindao@telconet.ec>
  * @version 1.0 12/11/2021
  *
  * @param Pr_DatosOC IN NAF47_TNET.CORREO_ORDEN.Gr_DatosOrdenCompra Recibe variable tipo registro para realizar proceso notificacion
  */
...
CREATE OR REPLACE PACKAGE BODY NAF47_TNET.CORREO_ORDEN IS
  --
  ...
  PROCEDURE SIGUIENTE_APROBADOR(Pv_IdEmpresa    IN VARCHAR2,
    ...
    Le_Error EXCEPTION;
  BEGIN
      CORREO_ORDEN.SIGUIENTE_APROBADOR@GPOETNET(Pv_IdEmpresa,
                                      Pv_IdOrden,
                                      Pv_IdTipoOrden,
                                      Pv_CodigoError,
                                      Pv_MensajeError);
  ...
  BEGIN
    ...
    IF Lv_Notifica = 'S' THEN
      ...
      IF Pv_IdOrden IS NULL THEN
        Pv_MensajeError := 'El Codigo de Orden no puede ser Nulo.';
        RAISE Le_Error;
      END IF;
      --
      IF Pv_IdTipoOrden IS NULL THEN
        Pv_MensajeError := 'El Codigo del Tipo de Orden no puede ser Nulo.';
        RAISE Le_Error;
      END IF;
      --
      ...
      <br>
      <span style="font-size:14px;"><span style="font-family:arial,helvetica,sans-serif;"><strong>Observacion:</strong>' || UPPER(Lr_Orden.OBSERV) || ' </span></span></p>
      <br>
    ...
    DB_GENERAL.GNRLPCK_UTIL.INSERT_ERROR('NAF',
      ...
      begin
      utl_mail.send(sender     => Lr_EmpleadoEnvia.Mail_Cia,--Lr_EmpleadoEnvia.MAIL, --
...
<td><span style="font-size:12px;"><span style="font-family:arial,helvetica,sans-serif;">' || Lv_NombreProveedor || '</span></span>
  ...
  BEGIN
    ...
    Lv_MensajeCorreo := Lv_MensajeCorreo ||     '<span style="font-size:14px;">';
    Lv_MensajeCorreo := Lv_MensajeCorreo ||      '<span style="font-family:arial,helvetica,sans-serif;">';
    Lv_MensajeCorreo := Lv_MensajeCorreo ||        '<strong>Observacion:</strong> '|| UPPER(Pr_DatosOC.OBSERVACION);
    Lv_MensajeCorreo := Lv_MensajeCorreo ||      '</span>';
    Lv_MensajeCorreo := Lv_MensajeCorreo ||    '</span>';
    --Lv_MensajeCorreo := Lv_MensajeCorreo ||     '</p>';
    ...
    IF Lr_Solicitante.Mail_Cia IS NULL THEN
      ...
      --JXZURITA email generico [11/11/2022] [FIN]
      IF Lr_Solicitante.Mail_Cia IS NULL THEN
      Lv_MensajeError := 'No se encuentra definido codigo de solicitante '||Pr_DatosOC.NO_SOLICITANTE||' para empresa '||Pr_DatosOC.NO_CIA;
        RAISE Le_Error;
      END IF;
    --JXZURITA email generico [11/11/2022] [INICIO]
    END IF;
    ...
    --JXZURITA email generico [11/11/2022] [FIN]
      IF Lr_DatosCrea.Mail_Cia IS NULL THEN
      Lv_MensajeError := 'No se encuentra definido codigo de usuario '||Pr_DatosOC.USUARIO||' para empresa '||Pr_DatosOC.NO_CIA;
        RAISE Le_Error;
      END IF;
    --JXZURITA email generico [11/11/2022] [INICIO]
    END IF;
    ...
    SYS.UTL_MAIL.SEND(sender     => 'naf@telconet.ec',
                      ...
                      subject    => 'SISTEMA NAF: AUTORIZACION COMPLETA DE ORDEN COMPRA NO.' || Pr_DatosOC.NO_ORDEN,
                      mime_type  => 'text/html; charset=us-ascii',
