CREATE OR REPLACE package            nomina_empleados is

 procedure carga_empleados(pcia varchar2,pplanilla varchar2,ppuesto varchar2,
                           parea varchar2,pdepa varchar2, pdivi varchar2, pseccion varchar2);

procedure ETIQUETA_COLUMNA(pcolumna number, pemple varchar2, pmonto VARCHAR2);
procedure VALOR_COLUMNA(PCIA VARCHAR2,PPLA VARCHAR2,pcolumna number, pemple varchar2, pmonto VARCHAR2);

end nomina_empleados;
/


CREATE OR REPLACE package body            nomina_empleados is
 
 procedure carga_empleados(pcia varchar2,pplanilla varchar2,ppuesto varchar2,
                           parea varchar2,pdepa varchar2, pdivi varchar2, pseccion varchar2) is

cursor c_emple is
select no_cia,cod_pla,no_emple
     from cg_nomina_en_proceso a
     ...
     group by no_cia,cod_pla,no_emple;
 

     
   cursor c_eti_ing_can is  
         select distinct(no_ingre)
         from cg_nomina_en_proceso 
         where no_cia like decode(pcia, 'T', '%',pcia)
         and cod_pla like decode(pplanilla, 'T', '%',pplanilla)
         and indicador = 'I'
         and no_ingre in
...
cursor c_INGRESOS is  
        ...
        ( select no_ingre
             from arplmi
             where no_cia like decode(pcia, 'T', '%',pcia)--'01'
             and (dep_sal_hora= 'SD' 
             or dep_sal_hora='S'))
         order by no_ingre;     
...
cursor c_tot_ingresos(pcia varchar2, ppla varchar2,pemple varchar2) is
       SELECT SUM(MONTO)
       FROM cg_nomina_en_proceso 
...
cursor c_tot_egresos(pcia varchar2, ppla varchar2,pemple varchar2) is
       SELECT SUM(MONTO)
       FROM cg_nomina_en_proceso 
        WHERE NO_EMPLE = pemple
        AND INDICADOR = 'D'
...
cursor c_tot_provisiones(pcia varchar2, ppla varchar2,pemple varchar2) is
       SELECT SUM(MONTO)
       FROM cg_nomina_en_proceso 
        WHERE NO_EMPLE = pemple
        AND INDICADOR = 'D'
...
--- EMPLEADO PARA ETIQUETAS   
    INSERT INTO NOMINA (EMPLEADO) VALUES ('NULO');
---- TODOS LOS EMPLEADOS    
    for i in c_emple loop
       insert into nomina (no_cia,planilla,empleado) values(i.no_cia,i.cod_pla,i.no_emple);
...
--------------------   ETIQUETAS   
    ...
    for k in c_eti_ing_can loop 
       nomina_empleados.etiqueta_columna(columna,'NULO',K.NO_INGRE);
       COLUMNA:=COLUMNA+1;
       nomina_empleados.ETIQUETA_columna(columna,'NULO','CANTIDAD');
       COLUMNA:=COLUMNA+1;
    END LOOP;
 
     for L in c_eti_ing  loop 
      
       nomina_empleados.ETIQUETA_columna(columna,'NULO',L.NO_INGRE);
       COLUMNA:=COLUMNA+1;
    END LOOP;
    nomina_empleados.ETIQUETA_columna(columna,'NULO','TOT_INGRESOS');
       COLUMNA:=COLUMNA+1;

    for e in c_eti_egreso_n  loop 
    
       nomina_empleados.ETIQUETA_columna(columna,'NULO',e.NO_INGRE);
       COLUMNA:=COLUMNA+1;
    END LOOP;
        nomina_empleados.ETIQUETA_columna(columna,'NULO','TOT_EGRESOS');
         COLUMNA:=COLUMNA+1;
         nomina_empleados.ETIQUETA_columna(columna,'NULO','NETO');
         COLUMNA:=COLUMNA+1;
    for d in c_eti_egreso_s  loop 
       nomina_empleados.ETIQUETA_columna(columna,'NULO',d.NO_INGRE);
       COLUMNA:=COLUMNA+1;
    END LOOP;
        nomina_empleados.ETIQUETA_columna(columna,'NULO','TOT_PROVISIONES');
       COLUMNA:=COLUMNA+1;
------------------------------
       COLUMNA:=1;
-----------------------------       
--------------- INGRESOS TIPO S o SD
    FOR I IN c_INGRESOS_CAN LOOP
      if bandera =1 then
      ingreso:= i.no_ingre;
      bandera:=2;
     end if; 
    if ingreso=i.no_ingre then
        nomina_empleados.VALOR_columna(i.no_cia,i.cod_pla,columna,I.NO_EMPLE,I.MONTO);
        total_ingresoc:=total_ingresoc+i.monto;

        COLUMNA:=COLUMNA+1;
        nomina_empleados.VALOR_columna(i.no_cia,i.cod_pla,columna,I.NO_EMPLE,I.CANTIDAD);
       columna:=columna-1;
...
------------------ TOTALES
   for k in c_emple loop
     open c_tot_ingresos(k.no_cia,k.cod_pla,k.no_emple);
     fetch c_tot_ingresos into total_ingreso;
     close c_tot_ingresos;
     nomina_empleados.valor_columna(k.no_cia,k.cod_pla,col_ingreso,k.NO_EMPLE,total_ingreso);
     
     open c_tot_egresos(k.no_cia,k.cod_pla,k.no_emple);
     fetch c_tot_egresos into total_egreso;
     close c_tot_egresos;
     nomina_empleados.valor_columna(k.no_cia,k.cod_pla,col_egreso,k.NO_EMPLE,total_egreso);
     
     open c_tot_provisiones(k.no_cia,k.cod_pla,k.no_emple);
     fetch c_tot_provisiones into total_provision;
     close c_tot_provisiones;
     nomina_empleados.valor_columna(k.no_cia,k.cod_pla,col_provision,k.NO_EMPLE,total_provision);
     
---------------  TOTAL NETO
   total_neto:=total_ingreso-total_egreso;
    nomina_empleados.valor_columna(k.no_cia,k.cod_pla,col_neto,k.NO_EMPLE,total_neto);     
...
procedure VALOR_COLUMNA(PCIA VARCHAR2,PPLA VARCHAR2,pcolumna number, pemple varchar2, pmonto VARCHAR2) is 
  begin
     if pcolumna=1 then
        UPDATE NOMINA 
        SET VALOR1 = PMONTO
