CREATE OR REPLACE PACKAGE            INV_PK_RECU_MASTER AS
...
CREATE OR REPLACE PACKAGE BODY            INV_PK_RECU_MASTER AS
  PROCEDURE PRC_GUARDA_HISTORICO(PN_COMPANIA       NAF47_TNET.INV_ARTI_RECU_MASTER.NO_CIA%TYPE,
                                 PV_COD_ARTICULO   NAF47_TNET.INV_ARTI_RECU_MASTER.NO_ARTI%TYPE,
                                 PN_ID_REGION_OLD  NAF47_TNET.INV_ARTI_RECU_MASTER.ID_REGION%TYPE,
                                 PV_COD_BODEGA_OLD NAF47_TNET.INV_ARTI_RECU_MASTER.BODEGA%TYPE,
  ...
  EXCEPTION
    WHEN OTHERS THEN
      rollback;
      PV_ERROR := 'Se presentó un error al registrar el artículo. ' ||
                  sqlerrm;
  END;
    ...
    IF PV_ACCION IS NULL THEN
      ...
      IF NOT LB_EXISTE THEN
        ...
        IF LB_EXISTE THEN
          ...
          COMMIT;
        ELSE
          PV_ERROR := 'Aún no se ha generado una nueva etapa de revisión.';
        END IF;
      END IF;
    END IF;
  END;
  ...
  BEGIN
    ...
    IF LB_EXISTE_PAR THEN
      ...
      EXCEPTION
        WHEN OTHERS THEN
          PN_ERROR := -1;
          PV_ERROR := 'Se presentó un problema al enviar mail.';
      END;
    END IF;
  END;
  ...
  BEGIN
    ...
    PV_MSG := 'Las Solicitudes de Pedidos se generaran el día ' ||
              to_char(sysdate, 'dd/mm/yyyy') || ' a las ' ||
              PV_HORA_EJECUCION;
  EXCEPTION
    WHEN OTHERS THEN
      PV_ERROR := 'Error al programar proceso de Generación de Solicitudes de Pedido. Contactarse con Sistemas.';
