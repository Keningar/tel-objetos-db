CREATE OR REPPLACE package NAF47_TNET.INK_DEVOLUCION_PEDIDO is
...
* Variable Registro que permite pasar por parametro los datos necesarios para procesar ingreso a bodega por devoluvion de pedidos
* @author llindao <llindao@telconet.ec>
* @version 1.0 23/10/2017
*/
...
*/
  ...
  * Procedure que registra Ingresos a bodegas por devolucion de pedidos.
  * @author llindao <llindao@telconet.ec>
  * @version 1.0 23/10/2017
  *
  * @author llindao <llindao@telconet.ec>
  * @version 1.1 13/08/2018  Se modifica para cambiar validacion de numeros de series para considerar unidades por serie
  *
  * @author llindao <llindao@telconet.ec>
  ...
  *
  * @author llindao <llindao@telconet.ec>
  * @version 1.4 30/07/2021 - Se modifica para considerar nuevo proceso de generacion numero de series
  *
  * @author banton <banton@telconet.ec>
  * @version 1.5 27/05/2022 - Se cambia validacion de numero de serie
  *
  * @author Elvis Mu?oz <emunoz@telconet.ec>
  * @version 1.6 15/01/2022 -Se procede al reemplazo de la funcion de LOGIN
...
/
CREATE OR REPLACE package body NAF47_TNET.INK_DEVOLUCION_PEDIDO is
  ...
  */
  PROCEDURE P_PROCESA_SERIE ( Pr_DocSerie     IN     NAF47_TNET.INV_PRE_INGRESO_NUMERO_SERIE%ROWTYPE,
    ...
    Le_Error EXCEPTION;
    --
    Ln_secuencia        number(19):=NAF47_TNET.MIG_SECUENCIA.SEQ_INV_PRE_INGRESO_SERIE;
  BEGIN
...
--                                           'pr_artdevolver('||Li_Devolucion||').PRODUCTO_USADO_ID = '|| pr_artdevolver(Li_Devolucion).PRODUCTO_USADO_ID||';'||CHR(10)||
      ...
      IF Pr_ArtDevolver(Li_Devolucion).ACCION != 'Devolver' THEN
        Pr_ArtDevolver(Li_Devolucion).DETALLE_ERROR := 'Tipo de accion no valido '||Pr_ArtDevolver(Li_Devolucion).ACCION;
        Pv_MensajeError := 'Tipo de accion no valido '||Pr_ArtDevolver(Li_Devolucion).ACCION;
        Raise Le_Error;
      END IF;
      --
      Lv_ArticuloNuevo  := 'NO';
      ...
      IF Pr_ArtDevolver(Li_Devolucion).MANEJA_SERIE = 'S' THEN
        ...
        IF NVL(Lr_ValidaCantSerie.Unidades ,0) != NVL(Pr_ArtDevolver(Li_Devolucion).CANTIDAD,0) THEN -- valida serie
          Pr_ArtDevolver(Li_Devolucion).DETALLE_ERROR := 'Para el articulo '||Pr_ArtDevolver(Li_Devolucion).PRODUCTO_ID||' cantidad de numero de series seleccionados '||Lr_ValidaCantSerie.Unidades||' no coincide con la cantidad a devolver '||NVL(Pr_ArtDevolver(Li_Devolucion).CANTIDAD,0)||'.';
          Pv_MensajeError := 'Para el articulo '||Lr_Arinml.No_Arti||' cantidad de numero de series seleccionados '||Lr_ValidaCantSerie.Unidades||' no coincide con la cantidad a devolver '||NVL(Pr_ArtDevolver(Li_Devolucion).CANTIDAD,0)||'.';
          Raise Le_Error;
        END IF;
      END IF;
      --
      ...
      END IF;
      --
      -- se recuperan las cantidades del detalle de la devolucion para validarlas
      ...
      IF C_CANTIDADES_DEVOLUCION%NOTFOUND THEN
        Pv_MensajeError := 'No se encontro cantidades a devolver para validar. Id Det Devolucion '||Pr_ArtDevolver(Li_Devolucion).ID_DEVOLUCIONES_DET;
        RAISE Le_Error;
      END IF;
      CLOSE C_CANTIDADES_DEVOLUCION;
      -------------------------------
      ...
      -------------------------------
      -- Se valida que los montos del detalle devolucion no fueron superados
      IF (Lr_CantDetDevol.Cantidad_Devuelta + Lr_Arinml.Unidades) > Lr_CantDetDevol.Cantidad_a_Devolver THEN
        Pv_MensajeError := 'Total unidades devueltas '||(Lr_CantDetDevol.Cantidad_Devuelta + Lr_Arinml.Unidades)||
                           ...
                           ' para el detalle devolucion '||Pr_ArtDevolver(Li_Devolucion).ID_DEVOLUCIONES_DET;
        RAISE Le_Error;
      ...
      UPDATE DB_COMPRAS.INFO_DEVOLUCIONES_DETALLE A
         SET A.ESTADO = Lv_EstDetalleDev,
           ...
           A.USR_ULT_MOD = LOWER(USER), --NAF47_TNET.GEK_CONSULTA.F_RECUPERA_LOGIN,                                      emunoz 12012023
      ...
      IF Pr_ArtDevolver(Li_Devolucion).CANTIDAD_DESPACHADA < (Pr_ArtDevolver(Li_Devolucion).CANTIDAD_DEVUELTA + Lr_Arinml.Unidades) THEN
        Pv_MensajeError := 'Total unidades devueltas '||(Pr_ArtDevolver(Li_Devolucion).CANTIDAD_DEVUELTA + Lr_Arinml.Unidades)||
                           ...
                           ' de la devolucion '||Pr_ArtDevolver(Li_Devolucion).ID_DEVOLUCIONES;
        RAISE Le_Error;
    ...
    INK_PROCESA_PEDIDOS.P_REGISTRA_TIEMPOS_PEDIDO(Pr_ArtDevolver(1)
                                                  ...
                                                  --GEK_CONSULTA.F_RECUPERA_LOGIN,          emunoz 12012023
                                                  LOWER(USER),                                     --emunoz 12012023
                                                  Lv_IdDetalles,
                                                  Pv_MensajeError,
                                                  Ln_IdLog);
    Pn_IdLog := Ln_IdLog;
    ...
    CURSOR C_NUMERO_SERIE IS
      SELECT COMPANIA,
       ...
       WHERE SERIE  = Pv_NumeroSerie
         ...
         --se valida si articulo no genera serie automatica.
    --
  ...
  BEGIN
    ...
    IF Lr_DatosActivo.No_Acti IS NOT NULL THEN
      Pv_MensajeError := 'Numero de serie '||Pv_NumeroSerie||' pertenece a activo fijo '||Lr_DatosActivo.No_Acti||' dado de Baja en la empresa '||Lr_DatosActivo.No_Cia;
      RAISE Le_Error;
    END IF;
    --
    ...
    IF Pv_MensajeError IS NOT NULL THEN
      Pv_MensajeError := 'No. Serie '||Pv_NumeroSerie||' se encuentra en Repositorio '||chr(13)||Pv_MensajeError||', Favor regule repositorio instalacion';
      RAISE Le_Error;
    END IF;
