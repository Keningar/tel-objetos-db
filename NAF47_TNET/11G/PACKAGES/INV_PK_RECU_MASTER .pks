...
CREATE OR REPLACE PACKAGE BODY NAF47_TNET.INV_PK_RECU_MASTER AS
  ...
  BEGIN
    ...
    INV_PK_RECU_MASTER.PRC_GUARDA_ARTICULO_POST@GPOETNET(PV_COMPANIA => PV_COMPANIA,
                                              PV_COD_ARTICULO => PV_COD_ARTICULO,
                                              PN_ID_REGION => PN_ID_REGION,
                                              PN_STOCK_MIN => PN_STOCK_MIN,
                                              PN_STOCK_MAX => PN_STOCK_MAX,
                                              PV_MOTIVO => PV_MOTIVO,
                                              PV_ERROR => PV_ERROR);
  ...
  */
  --PRAGMA AUTONOMOUS_TRANSACTION;
  BEGIN
    INV_PK_RECU_MASTER.PRC_REPLICA_RECU_TO_TMP@GPOETNET(PV_EMPRESA => PV_EMPRESA,
                                                        PV_ACCION => PV_ACCION,
                                                        PV_ERROR => PV_ERROR);
    commit;
    /*
    IF PV_ACCION IS NULL THEN
      ...
      IF NOT LB_EXISTE THEN
        ...
        IF LB_EXISTE THEN
          ...
          COMMIT;
        ELSE
          PV_ERROR := 'A\FAn no se ha generado una nueva etapa de revisi\F3n.';
        END IF;
      END IF;
    END IF;
    */
  exception
    when others then
      rollback;
      PV_ERROR := 'Error al programar proceso de Generaci\F3n de Solicitudes de Pedido. Contactarse con Sistemas.';
  END;
  ...
  INV_PK_RECU_MASTER.PRC_REPROGRAMAR_JOB@GPOETNET(PV_HORA_EJECUCION => PV_HORA_EJECUCION,
                                                  PV_MSG => PV_MSG,
                                                  PV_ERROR => PV_ERROR);
   --commit;
     /*
    ...
    DBMS_SCHEDULER.enable(name => '"NAF47_TNET"."JOB_ARTICULO_RECURRENTE"');
    DBMS_SCHEDULER.set_attribute_null(name      => '"NAF47_TNET"."JOB_ARTICULO_RECURRENTE"',
                                      attribute => 'repeat_interval');
    DBMS_SCHEDULER.set_attribute(name      => '"NAF47_TNET"."JOB_ARTICULO_RECURRENTE"',
                                 attribute => 'start_date',
                                 value     => TO_TIMESTAMP_TZ(LV_FECHA,
