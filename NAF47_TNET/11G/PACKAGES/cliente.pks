...
IS
  ...
  * @version 1.2 04/04/2022 - se modifica para agregar schema DB_COMERCIAL 
  */
...
--
  ...
  BEGIN
      ...
      IF NAF47_TNET(gTstamp             IS NULL OR ABS(vtstamp - gTstamp) > 2) OR
...
BEGIN
  ...
  IF Pv_TipoIdTributacio = 'C' AND NVL(Lv_parametro,'N')='N' THEN
    DB_COMERCIAL.VALIDA_IDENTIFICACION.VALIDA('C', Pv_Cedula, Lv_error);
  ELSIF Pv_TipoIdTributacio = 'R' AND NVL(Lv_parametro,'N')='N' THEN
    DB_COMERCIAL.VALIDA_IDENTIFICACION.VALIDA('R', Pv_Cedula, Lv_error);
  END IF;
