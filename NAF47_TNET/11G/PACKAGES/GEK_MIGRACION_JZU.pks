CREATE OR REPLACE package            GEK_MIGRACION_JZU is
  ...
  */
  PROCEDURE RHP_REGISTRA_HIST_DE_REINGRESO(Pv_NoCia   IN VARCHAR2,
                                           ...
                                           Pv_Salida  OUT VARCHAR2,
                                           Pv_Mensaje OUT VARCHAR2);
end GEK_MIGRACION_JZU;
/


CREATE OR REPLACE package body            GEK_MIGRACION_JZU is
  ...
  EXCEPTION
    WHEN Le_Error THEN
      ...
      DB_GENERAL.GNRLPCK_UTIL.INSERT_ERROR('NAF',
                                           ...
                                           Pv_MensajeError,
                                           ---GEK_CONSULTA.F_RECUPERA_LOGIN,              emunoz 110125023
                                           LOWER(USER),                                            -- emunoz 11012023
                                           SYSDATE,
                                           GEK_CONSULTA.F_RECUPERA_IP);
    WHEN OTHERS THEN
  ...
  BEGIN
    ...
    IF C_PUESTODESC%NOTFOUND THEN
      ...
      VALUES
        (Pv_NoCia, NAF47_TNET.MIG_SECUENCIA.SEQ_ARPLMP, Pv_Puesto, Pv_EsJefe);
      Lb_Nuevo := TRUE;
    END IF;
    CLOSE C_PUESTODESC;
...
RHP_REGISTRA_HIST_DE_REINGRESO(Pv_NoCia,
  ...
  PROCEDURE P_ELIMINA_MIGRA_CG(Pd_Fecha        IN DATE,
    ...
    -- @CostoQuery: 2216
    CURSOR C_VERIFICA_MAYORIZADOS IS
      SELECT A.NO_ASIENTO
    ...
    -- @CostoQuery: 604
    CURSOR C_VERIFICA_AUTORIZADOS IS
      SELECT A.NO_ASIENTO
  ...
  BEGIN
    ...
    FOR Li_Mayorizado IN C_VERIFICA_MAYORIZADOS LOOP
    ...
    FOR Li_Autorizado IN C_VERIFICA_AUTORIZADOS LOOP
      -- Se concatenan los asientos mayorizados encontrados
      IF Pv_MensajeError IS NULL THEN
    ...
    FOR Li_Contab IN C_CONTABILIZADOS LOOP
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      DB_GENERAL.GNRLPCK_UTIL.INSERT_ERROR('NAF',
                                           ...
                                           Pv_Mensaje,
                                           --GEK_CONSULTA.F_RECUPERA_LOGIN,    11012023
                                           USER,                               --emunoz 11012023
                                           SYSDATE,
                                           GEK_CONSULTA.F_RECUPERA_IP);
  ...
  BEGIN
    ...
    IF C_CLIENTES%NOTFOUND THEN
      Lv_MensajeError := ' No se encontro Cliente en C_CLIENTES: ' ||
                         Pv_TipoIdentificacion;
      Raise Le_Error;
    END IF;
    CLOSE C_CLIENTES;
    ...
    IF C_PERIODO%NOTFOUND THEN
      Lv_MensajeError := ' No se encontro Periodo en C_PERIODO';
      Raise Le_Error;
    END IF;
    CLOSE C_PERIODO;
    ...
    IF C_SUBCLIENTE%NOTFOUND THEN
      Lv_MensajeError := ' No se encontro Subcliente en C_SUBCLIENTE';
      Raise Le_Error;
    END IF;
    CLOSE C_SUBCLIENTE;
    ...
    IF C_COBRADOR%NOTFOUND THEN
      Lv_MensajeError := ' No se encontro Cobrador en COBRADOR';
      Raise Le_Error;
    END IF;
    CLOSE C_COBRADOR;
    ...
    IF C_AUTOMATICO%NOTFOUND THEN
      Lv_MensajeError := ' No se encontro Formulario en C_AUTOMATICO';
      Raise Le_Error;
    END IF;
    ...
    IF C_FORMU%NOTFOUND THEN
      Lv_MensajeError := ' No se encontro Secuencia en C_FORMU';
      Raise Le_Error;
    END IF;
    CLOSE C_FORMU;
    ...
    IF C_RETE%NOTFOUND THEN
      Lv_MensajeError := ' No se encontro Tipo Retencion en C_RETE';
      Raise Le_Error;
    END IF;
    CLOSE C_RETE;
    ...
    IF C_FORMU_TIPO_DOC%NOTFOUND THEN
      Lv_MensajeError := ' No se encontro Tipo Documento en C_FORMU_TIPO_DOC';
      Raise Le_Error;
    END IF;
    CLOSE C_FORMU_TIPO_DOC;
    ...
    IF C_TIPO%NOTFOUND THEN
      Lv_MensajeError := ' No se encontro Tipo  en C_TIPO';
      Raise Le_Error;
    END IF;
    CLOSE C_TIPO;
      ...
      FOR I in 1 .. Ln_ContadorRetencion LOOP
        ...
        IF Lr_ARCCTI.Clave IS NOT NULL THEN
          ...
          IF C_ARCGIMP%NOTFOUND THEN
            Lv_MensajeError := ' No se encontro impuesto en C_ARCGIMP ';
            Raise Le_Error;
          END IF;
          CLOSE C_ARCGIMP;
          ...
          IF C_MONTO_RET%NOTFOUND OR NVL(Ln_Tot_Ret, 0) = 0 THEN
            Lv_MensajeError := ' No se encontro Total Ret para Cliente:' ||
    ...
    ELSE
      Lv_MensajeError := 'Debito: ' || Ln_SumDebito || ' y  Credito: ' ||
