create or replace procedure            INACT_CONTROL_OTRA_EMPRESA(cia_p         IN varchar2,
                                                       tipo_p        IN varchar2,
                                                       docu_p        IN varchar2,
  ...
  --  Su funcion es actualizar las unidades de articulos de otra empresa pero que no afecte al costo
  -- Se modifica para que la cuenta contrapartida la tome de lo configurado en la cuenta ARINVTM.
  -- Lee el documento vario que esta pendiente de actualizar.
  ...
  CURSOR c_lotes (cia_c  varchar2,
                  ...
                  doc_c  varchar2,
                  lin_c  number) IS
    SELECT no_lote, nvl(unidades,0) unidades, 0 monto, ubicacion, fecha_vence
      FROM arinmo
     WHERE no_cia   = cia_c
       AND no_docu  = doc_c
       AND tipo_doc = tip_c
  ...
  CURSOR C_PRODUCTO_SERIE IS
    SELECT A.NO_ARTI, A.UNIDADES, COUNT(B.SERIE) CANTIDAD
      FROM ARINML A,
           INV_DOCUMENTO_SERIE B,
           ARINDA C
...
BEGIN
  ...
  INcrea_encabezado_h(rd.rowid_me, rper.mes_proce, rper.semana_proce, rper.indicador_sem);  -- Historico
  --
  vmov_tot := 0;
  ...
  FOR i IN c_lineas_doc LOOP
     ...
     ****/
      ...
      -- El costo unitario solamente se actualiza cuando se trata de movimientos de entrada pero para este tipo
      -- solo se aplicar[a a la misma bodega
      /* MNA  actualizar este proceso para que distinga de la Bod de mal estado y la de Control (reserva)
         en las 2 partes del IF  PENDIENTE DE HACER   */
     ...
     INActualiza_saldos_articulo  (rd.no_cia, i.bodega, i.no_arti, 'PRODUCCION',
                                  (i.unidades * vsigno), (i.monto * vsigno), Null, msg_error_p);
     IF msg_error_p is not null THEN
       RAISE error_proceso;
     END IF;

     -- Este proceso se utiliza para actualizar los saldos valuados del costo y costo 2 del inventario ANR 27/04/2009
     /* ESTE PROCEDIMIENTO NO SE VA A UTILIZAR PORQUE ACTUALIZA EN TODAS LAS BODEGAS
...
EXCEPTION
  ...
  WHEN others THEN
       ...
       return;
end INACT_CONTROL_OTRA_EMPRESA;