create or replace procedure            CCDEBITO_DIFERENCIA (Pv_cia             In Varchar2,
                                                 Pv_centro          In Varchar2,
                                                 Pv_secuencia_lote  In Varchar2,
                                                 Pv_secuencia       In Varchar2,
                                                 Pv_no_docu         In Varchar2,
                                                 Pvano_proce_cxc    In Varchar2,
                  															 Pvmes_proce_cxc    In Varchar2,
                  															 Pvsemana_proce_cxc In Varchar2,
                  															 Pvdia_proceso_cxc  In Varchar2,
...
--Modificado por : Martha Navarrete M.
--27-09-2010
-- Correccion en valores de asignacion.
...
Cursor C_cuenta_contable (cv_cia   In Varchar2,
                          Cv_fpago In Varchar2) Is
 Select cuenta_contable
    From arccforma_pago
   Where no_cia = cv_cia
     And forma_pago = Cv_fpago;
...
Lv_cta_contable              arccforma_pago.cuenta_contable%Type;
         ...
         If nvl(i.valor_confirmado,0) != nvl(i.valor_linea,0) Then
           ...
           Begin
             ...
             INSERT INTO arccmd(no_cia,            centro,        tipo_doc,        periodo,              ruta,              no_docu,
                        ...
                        VALUES (Pv_cia,            Pv_centro,     Lc_tipo.tipo,    Pvano_proce_cxc,       '0000',            Lv_no_docu,
                                r.grupo,           r.no_cliente,  r.moneda,        Pvdia_proceso_cxc,     i.fecha_vigencia,  i.fecha_vigencia,
                                i.valor_linea,     0,             Ln_diferencia,   'V',                  0,                 Ln_diferencia,
                                0,                 0,             0,               r.no_agente,          'P',               r.tipo_cambio,
                                0,                 Ln_diferencia, Ln_diferencia,   'CC',                 Pvano_proce_cxc,    Pvmes_proce_cxc,
                                Pvsemana_proce_cxc, i.fecha_cheque,  '0',             Lc_tipo.Cod_Diario, Pv_no_docu,        user,
           ...
           Exception
               When Others Then
                  Pv_error := 'Error al crear documento de Debito '||sqlerrm;
                  raise Le_error;
           End;
          ...
          If cuenta_contable.acepta_cc (Pv_cia, vCta_Cliente) THEN
            Lv_cc := CC_CCOSTO_SUBCLIENTE(Pv_cia, r.grupo ,r.no_cliente, r.sub_cliente);
            If Lv_cc is null Then
            	Pv_error := 'El cliente: '||r.grupo||' '||r.no_cliente||' subcliente: '||r.sub_cliente||' no tiene configurado centro de costos';
            	raise Le_error;
            else
          ...
          Begin
            Insert into ARCCDC (no_cia,        centro,          tipo_doc,         periodo,        ruta,          no_docu,
                                ...
                                glosa)
                        Values (Pv_cia,        r.centro,        Lc_tipo.tipo,     Pvano_proce_cxc, r.ruta,        Lv_no_docu,
                                r.grupo,       r.no_cliente,    vCta_cliente,     'D',            Ln_diferencia, Ln_diferencia/r.tipo_cambio,
                                r.tipo_cambio, r.moneda,        'P',              Lv_centro_costo,    'N',           Ln_diferencia,
          ...
          Open C_cuenta_contable(Pv_cia, i.forma_pago);
          Fetch C_cuenta_contable Into Lv_cta_contable;
          Close C_cuenta_contable;

          If cuenta_contable.acepta_cc (Pv_cia, Lv_cta_contable) THEN
            Lv_cc := CC_CCOSTO_SUBCLIENTE(Pv_cia, r.grupo ,r.no_cliente, r.sub_cliente);
            If Lv_cc is null Then
            	Pv_error := 'El cliente: '||r.grupo||' '||r.no_cliente||' subcliente: '||r.sub_cliente||' no tiene configurado centro de costos';
            	raise Le_error;
          ...
          Begin
            Insert into ARCCDC (no_cia,        centro,          tipo_doc,          periodo,         ruta,          no_docu,
                               ...
                               glosa)
                       Values (Pv_cia,         r.centro,        Lc_tipo.tipo,      Pvano_proce_cxc,  r.ruta,        Lv_no_docu,
                               r.grupo,        r.no_cliente,    Lv_cta_contable,   'C',             Ln_diferencia, Ln_diferencia/r.tipo_cambio,
                               r.tipo_cambio,  r.moneda,        'P',               Lv_centro_costo, 'N',           Ln_diferencia,
                               r.no_cliente||' - '||Lc_tipo.tipo||' -  DOC: '||Pv_no_docu);
          Exception
            When Others Then
