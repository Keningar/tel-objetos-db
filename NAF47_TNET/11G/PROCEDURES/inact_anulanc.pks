create or replace PROCEDURE            inact_anulanc  (cia_p        IN varchar2,
                                            tipo_p       IN varchar2,
                                            docu_p       IN varchar2,
  ...
  /*** Este proceso se lo llama para anular las notas de credito por devolucion de productos
       El movimiento a realizar es parecido a un despacho por ventas ***/
...
EXCEPTION
  WHEN error_proceso THEN
     msg_error_p := NVL(msg_error_p, 'error_proceso en inact_anulanc');
     RETURN;
  WHEN OTHERS THEN
     msg_error_p := 'INACT_ANULANC : ' || SQLERRM;
     RETURN;
END;