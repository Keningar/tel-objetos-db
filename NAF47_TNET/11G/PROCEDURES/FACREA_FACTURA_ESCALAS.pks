create or replace procedure            FACREA_FACTURA_ESCALAS (
  ...
  CURSOR c_lineas IS
    SELECT no_cia,      centrod,     no_linea,    bodega,
     ...
     WHERE no_cia   = no_cia_p
       ...
       AND cant_aprobada > 0
       and exists
       (select no_cia from arfapromo_flc z
...
BEGIN
  ...
  If C_Pedido%notfound Then
   Close C_Pedido;
       mensaje_p := 'FACREA_FACTURA_ESCALAS: Pedido no existe: '||no_pedido_p;
       RAISE error_proceso;
  else
   Close C_Pedido;
  end if;
  ...
  If C_Dia_proceso%notfound Then
   Close C_Dia_proceso;
       mensaje_p := 'FACREA_FACTURA_ESCALAS: No existe centro de distribucion: '||Lv_centro_distribucion;
       RAISE error_proceso;
  else
   Close C_Dia_proceso;
  end if;
  ...
  If Ld_dia_proceso is null Then
     mensaje_p := 'FACREA_FACTURA_ESCALAS: No existe dia de proceso de facturacion para el centro de distribucion: '||Lv_centro_distribucion;
     RAISE error_proceso;
  end if;
  ...
  if c_lineas%found then
     lb_crea_guia:=true;
      ...
      Exception
        When others Then
           mensaje_p := 'FACREA_FACTURA_ESCALAS: Error al crear cabecera de factura: '||no_factu_p||' '||sqlerrm;
           RAISE error_proceso;
      end;
  end if;
  ...
  If nvl(l.cant_aprobada,0) > nvl(l.pedido,0) + nvl(l.cantidad_adicional,0) Then
       mensaje_p := 'FACREA_FACTURA_ESCALAS: Para el articulo: '||l.no_arti||' la cantidad aprobada: '||nvl(l.cant_aprobada,0)||' no puede ser mayor a la cantidad pedida: '||Ln_total_pedido;
       RAISE error_proceso;
    ...
    IF nvl(vCosto,0) = 0 AND l.bodega != '0000' THEN
       mensaje_p := 'FACREA_FACTURA_ESCALAS: El articulo/servicio '||l.no_arti||' no tiene costo definido en la bodega '||l.bodega;
       RAISE error_proceso;
     END IF;
     ...
     INSERT INTO arfafl(no_cia,     centrod,   tipo_doc,    periodo,
      ...
      Exception
        When others Then
           mensaje_p := 'FACREA_FACTURA_ESCALAS: Error al crear linea de factura: '||no_factu_p||' linea: '||l.no_linea||' bodega: '||l.bodega||' articulo: '||l.no_arti||' '||sqlerrm;
           RAISE error_proceso;
      end;
   ...
   Insert into Arfapromo_fl (no_cia, no_factu, no_linea, secuencia_politica,
    ...
    Exception
      When others Then
         mensaje_p := 'FACREA_FACTURA_ESCALAS: Error al crear linea de promocion de factura: '||no_factu_p||' linea: '||i.no_linea||' '||sqlerrm;
         RAISE error_proceso;
    End;
  ...
  ---- Carga lotes en la tabla de temporal de lotes de la factura
    ...
    Exception
      When others Then
         mensaje_p := 'FACREA_FACTURA_ESCALAS: Error al crear linea de lote de factura: '||no_factu_p||' linea: '||lot.no_linea||' '||sqlerrm;
         RAISE error_proceso;
    End;
  ...
  FOR ii IN C_Arfafli Loop
      ...
      Fetch C_Impuestos into Ln_columna, Lv_clave, Ln_porcentaje;
       If C_Impuestos%notfound Then
        Close C_Impuestos;
        mensaje_p := 'FACREA_FACTURA_ESCALAS: El articulo/servicio '||ii.no_arti||' no tiene configurados impuestos ';
        RAISE error_proceso;
       else
        Close C_impuestos;
       end if;
       ...
       If Lv_imp_arti = 'S' and Lv_excento_cli = 'N' Then
            ...
            Exception
              When others Then
                 mensaje_p := 'FACREA_FACTURA_ESCALAS: Error al crear linea de factura (impuesto): '||no_factu_p||' linea: '||ii.no_linea||' '||sqlerrm;
                 RAISE error_proceso;
            end;
    ...
    For h in C_Actualiza_Arfafe (no_factu_p) Loop
        ...
        If C_Detalle%notfound Then
         Close C_Detalle;
           mensaje_p := 'FACREA_FACTURA_ESCALAS: Documento :'||h.no_factu||' sin detalle, no se puede procesar';
           RAISE error_proceso;
        else
         Close C_Detalle;
        end if;
