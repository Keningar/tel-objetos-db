create or replace procedure            FAGENERADC_DEVOLUCION(pV_Cia     IN arfafe.No_Cia%TYPE,
												 pv_centro    IN arfafe.Centrod%TYPE,pd_fecha_proceso date,
  ...
  CURSOR c_documentos IS
    SELECT f.tipo_doc,        f.periodo,     f.ruta,   f.no_factu,
           ...
           f.ind_exportacion, f.centrod, f.division_comercial, f.n_factu_d,
           f.rowid
      FROM arfact t, arfafe f
     WHERE f.no_cia  = pv_Cia
       AND f.estado  = 'D'
       AND NVL(ind_fac_dev,'N') = 'D'
  ...
  CURSOR c_lineas( pNo_factu varchar2) IS
  ...
  CURSOR c_total_bruto ( pTipo_doc varchar2, pNo_factu varchar2, pBodega varchar2, pGrupo varchar2) IS
    SELECT nvl(sum(abs(decode(l.i_ven,'N',nvl(l.total,0)))),0) base_exenta,
           nvl(sum(abs(decode(l.i_ven,'S',nvl(l.total,0)))),0) base_gravada
      FROM arinda d, arfafl l
     WHERE l.no_cia    = pv_cia
       AND l.no_factu  = pNo_factu
 ...
 --add mlopez 31/05/2010
  cursor c_factura(cv_factu varchar2)is
   select b.centro_costo
    from arfafe a,arfa_div_comercial b
   where a.no_cia=pv_cia
    and a.no_factu=cv_factu
    and a.no_cia=b.no_cia
    and a.division_comercial=b.division
  ...
  Ln_base_excenta    Arfadc.monto%type;
...
/*
	La generacion contable de la nota de credito por devolucion sera :
   1.- Devoluciones en Ventas al centro de costo de la division comercial del vendedor. (Debe). Esta cuenta se la toma de
       la parametrizacion por bodega. La  devolucion debe estar dividida en Devoluciones en Ventas base 0% y
       Devoluciones en Ventas base 12%
   2.- Se debe contabilizar la cuenta contable de Impuesto si la devolucion tiene items que pagan Iva.
   3.- Se debe contabilizar la cuenta Descuentos en Ventas si los items que se devuelven fueron facturados con descuentos.
   4.- Cuentas por Cobrar Clientes (Haber).
*/
  ...
  FOR doc IN c_documentos LOOP
      ...
      --  1.- OBTENGO LA CUENTA DEL CLIENTE (HABER)
      ...
      --  2.- Define la forma contable que tendra el asiento (Asiento de devolucion)
      ...
      if doc.n_factu_d is null then
      	 --- El centro de costo que recupera es el que esta configurado pora cada division comercial ANR 25/08/2009
		     ...
		     open c_factura(doc.n_factu_d);
		     fetch c_factura into vcentro_costo;
		     close c_factura;
      end if;
    ...
    FOR ld IN c_lineas(doc.no_factu) LOOP
      ...
      IF NOT inlib.trae_cuentas_conta(pv_cia, vGrupo_contable, vBodega, vrctas) THEN
         pv_error := 'No se pudo obtener las cuentas contables para el '||'grupo '||vGrupo_contable||', bodega: '||vBodega;
         RAISE error_proceso;
      END IF;
      ...
      IF (vcta_venta_gravada is null) THEN
		     IF doc.ind_fac_dev = 'D' THEN  -- se procesa una factura
			      pv_error := 'Falta definir la cuenta de Devoluciones gravada (contado/credito), para la bodega '||vBodega ||' y el grupo ' || vGrupo_contable;
		        RAISE error_proceso;
		     ELSE -- se procesa una devolucion
			      pv_error := 'Falta definir la cuenta de Ventas gravada (contado/credito), para la bodega '||vBodega ||' y el grupo ' || vGrupo_contable;
		        RAISE error_proceso;
		     END IF;

      ELSIF (vcta_venta_excenta is null) THEN
		     IF doc.ind_fac_dev = 'D' THEN  -- se procesa una factura
			      pv_error := 'Falta definir la cuenta de Devoluciones excenta (contado/credito), para la bodega '||vBodega ||' y el grupo ' || vGrupo_contable;
		        RAISE error_proceso;
		     ELSE -- se procesa una devolucion
			      pv_error := 'Falta definir la cuenta de Ventas excenta(contado/credito), para la bodega '||vBodega ||' y el grupo ' || vGrupo_contable;
		        RAISE error_proceso;
		     END IF;
      ELSIF (vcta_descuento_gravado is null) THEN
		     pv_Error := 'Falta definir la cuenta de Descuentos gravado, para la bodega '|| vBodega ||' y el grupo ' || vGrupo_contable;
		     RAISE error_proceso;
      ELSIF (vcta_descuento_excento is null) THEN
		     pv_error := 'Falta definir la cuenta de Descuentos excento, para la bodega '|| vBodega ||' y el grupo ' || vGrupo_contable;
		     RAISE error_proceso;
      END IF;
      -- ******************************
      ...
      Fetch c_total_bruto into Ln_base_excenta, Ln_base_gravada;
      ...
      inserta_arfadc(pv_cia,         pv_centro,          to_char(pd_fecha_proceso,'YYYY'),
                     to_char(pd_fecha_proceso,'MM'),doc.tipo_doc,      doc.periodo,
                     doc.ruta,        doc.no_factu,      doc.moneda,
                     vcta_cxc_caja,   nvl(nvl(ld.total,0) - nvl(ld.descuento, 0) + nvl(ld.i_ven_n, 0),0),
...
EXCEPTION
  ...
  WHEN error_proceso THEN
       pv_error := 'DC_DEVOLUCION (EP): '||nvl(pv_error,vError);
       return;
  WHEN others then
       pv_error := 'DC_DEVOLUCION: '||sqlerrm;
       return;

end FAGENERADC_DEVOLUCION;