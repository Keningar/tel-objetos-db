create or replace PROCEDURE            CCGenera_Conta (
  cia_p                 varchar2,
  centro_p              varchar2,
  ano_p                 number,
  mes_p                 number,
  ...
  CURSOR c_Diarios IS
    SELECT DISTINCT md.Cod_Diario
      FROM arccdc dc, arccmd md
     WHERE dc.no_cia       = cia_p
       AND dc.centro       = centro_p
       AND md.fecha        = fecha_p
       AND dc.ind_con      = 'P'
...
BEGIN  -- CCGenera_Conta
  ...
  If C_Centro%notfound Then
    ...
    IF vTipo_asiento = 'R' THEN
    	vDescri := 'ASIENTO RESUMIDO GENERADO DESDE CUENTAS POR COBRAR - FECHA : '||fecha_p||' CENTRO: '||centro_p||' '||LV_NOMBRE_CENTRO;
    ELSE -- vTipo_asiento = D
    	vDescri := 'ASIENTO DETALLADO GENERADO DESDE CUENTAS POR COBRAR - FECHA : '||fecha_p||' CENTRO: '||centro_p||' '||LV_NOMBRE_CENTRO;
    END IF;
      Begin
      INSERT INTO arcgae (NO_CIA,           ANO,        MES,
                  ...
                  VALUES (
                          ...
                          'T', '0',  'C',
                          substr(vDescri,1,240));
      Exception
      When Others Then
       	pError := 'Error al generar cabecera contable: '||vasiento||' '||sqlerrm;
       	RAISE error_proceso;
      end;
    vtot_db   := 0;
    vtot_cr   := 0;
    ...
    IF vTipo_asiento = 'R' THEN
      ...
      FOR fila IN c_mov_dc (vasiento) LOOP
        ...
        vDescri2 := substr('GENERADO POR MOD. DE CxC - FECHA : '||fecha_p,1,100);
     ...
     Begin
      ...
      Exception
      When Others Then
       	pError := 'Error al generar detalle contable: '||vasiento||' cuenta: '||' '||fila.codigo||' '||sqlerrm;
       	RAISE error_proceso;
      end;
    ...
    ELSE -- vTipo_asiento = 'D'
      ...
      FOR fila IN c_mov_dc_det (vasiento) LOOP
        ...
        vTipo_cambio := fila.tipo_cambio;
        vcontador := NVL(vcontador,0) + 1;
        vDescri2 := SUBSTR(fila.descri||' FECHA : '||fecha_p,1,100);
      Begin
          INSERT INTO arcgal(NO_CIA, ANO,            MES,
                    NO_ASIENTO,    NO_LINEA,       CUENTA,
                    MONTO,         TIPO,           COD_DIARIO,
      ...
      Exception
      When Others Then
       	pError := 'Error al generar el detalle contable: '||vasiento||' cuenta: '||' '||fila.codigo||' '||sqlerrm;
       	RAISE error_proceso;
      end;
