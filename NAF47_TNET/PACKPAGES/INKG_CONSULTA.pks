CREATE EDITIONABLE package INKG_CONSULTA is
/**
...
*/
  ...
  *
  * @author llindao <llindao@telconet.ec>
  * @version 1.1 14/04/2020 - Se agrega parametro login para identificar ubicaci�n de empleado y presentar articulos de bodega de misma localidad
  *
  * @param Pv_Articulo       IN VARCHAR2 Recibe decripci�n para filtro por art�culo
  * @param Pv_Grupo          IN VARCHAR2 Recibe decripci�n para filtro por grupo
  * @param Pv_SubGrupo       IN VARCHAR2 Recibe decripci�n para filtro por sub grupo
  ...
  * @return CLOB retorna JSON con datos de art�culos por caracteristicas
  */
  ...
  * @return CLOB retorna JSON con datos de art�culos por caracteristicas
  */
  FUNCTION F_GET_JSON_ARTICULOS_MM (
                                 Pv_modelo       VARCHAR2,
                                 Pv_marca    VARCHAR2,
                                 Pv_NoCia       VARCHAR2,
                                 Pv_Status  OUT VARCHAR2,
  ...
  /**
  * Documentacion para F_GET_JSON_MARCAS
  * Funcion que retorna JSON con los datos de marcae por compa�ia en NAF.
  * @author lardila <lardila@telconet.ec>
  * @version 1.0 20/05/2022
  *
  * @return CLOB retorna JSON con datos de departamentos
  */
  FUNCTION F_GET_JSON_ARTICULOS_MARCA (         Pv_NoCia       VARCHAR2,
                                       Pv_Status  OUT VARCHAR2,
                                       Pv_Mensaje OUT VARCHAR2) RETURN CLOB;
                                       
    /**
  * Documentacion para F_GET_JSON_MODELO
  * Funcion que retorna JSON con los datos de modelos por compa�ia y marca en NAF.
  * @author lardila <lardila@telconet.ec>
  * @version 1.0 20/05/2022
  *
  * @return CLOB retorna JSON con datos de departamentos
  */
  FUNCTION F_GET_JSON_MODELO (         Pv_Marca       VARCHAR2,
                                       Pv_NoCia       VARCHAR2,
                                       Pv_Status  OUT VARCHAR2,
                                       Pv_Mensaje OUT VARCHAR2) RETURN CLOB;
  ...
  * Funcion que retorna JSON con los datos de departamentos por compa�ia en NAF.
  * @author llindao <llindao@telconet.ec>
  ...
  * Funci�n que retorna JSON con los datos de grupos por compa�ia en NAF.
  ...
  * Funci�n que retorna JSON con los datos de los subgrupos por compa�ia en NAF.
  * @author Kevin Baque Puya <kbaque@telconet.ec>
  * @version 1.0 - 27/11/2020
  *
  ...
  * Funcion que retorna si n�mero de serie puede ser ingresado en bodega.
  * @author llindao <llindao@telconet.ec>
  * @version 1.0 25/01/2021
  *
  * @param Pv_NumeroSerie   IN VARCHAR2          N�mero de serie a validar
  * @param Pv_NoCia         IN VARCHAR2          C�digo que identifica a la empresa 
  ...
  * Funcion que retorna la descripcion total del detalle de formato n�mero serie.
  * @author llindao <llindao@telconet.ec>
  * @version 1.0 30/07/2021
  *
  * @param Pn_FormatoSerieId   IN VARCHAR2  Recibe c�digo de formato serie
...
CREATE EDITIONABLE package body             INKG_CONSULTA is
  --
  ...
  BEGIN
    --
    Pv_Status  := '200';
    Pv_Mensaje := 'Transacci�n realizada con �xito';
    --
    ...
    IF NVL(Ln_CantonLogin,0) = 0 THEN
      Pv_Status  := '404';
      Pv_Mensaje := 'No se encontr� ciudad a la que pertenece usuario '||Pv_Login;
      GOTO ET01_FIN_JSON;
    END IF;
    --
    ...
    IF Ln_Linea = 0 THEN
      Pv_Status  := '404';
      Pv_Mensaje := 'No se encontraron art�culos en localidad a la que pertenece el usuario '||Pv_Login;
    END IF;
    --
    <<ET01_FIN_JSON>>
    --
 ...
 FUNCTION F_GET_JSON_MODELO (          Pv_Marca       VARCHAR2,
        ...
        SELECT distinct 
               arin.marca brandId, 
               arin.modelo modelo
        FROM NAF47_TNET.arinda arin 
        WHERE arin.no_cia = id_code
        and arin.marca = pv_marcar;
    ...
    BEGIN
        ...
        FOR lr_datos IN C_MODELOS(Pv_Marca,Pv_NoCia) LOOP
            ...
            lv_regjson := '{'
                            ...
                            || '",'
                            || '"modelDescription":"'
                            || REPLACE(lr_datos.modelo,chr(34),' ')
                            || '"'
                            || 
                          '}';
...
FUNCTION F_GET_JSON_ARTICULOS_MM (
                ...
                FROM
                    (
                        SELECT
                            ...
                            arin.costo_unitario,
                            arin.unidad,
                            ma.estado_arti_activo,
                            ma.stock
                            FROM
                                naf47_tnet.v_articulos_caracteristicas ma,
                                naf47_tnet.arinda                      arin,
                                naf47_tnet.arincat                     arinc

                            WHERE
                                arin.no_cia = Pv_noCia
                                AND ma.no_arti = arin.no_arti
                                AND arinc.no_cia = arin.no_cia
                                AND arinc.clase = arin.clase
                                AND ma.estado_arti_activo = 'S'
                                AND arin.modelo = Pv_modelo
                                AND arin.marca = Pv_marca
                        ) x
                GROUP BY 
                     x.no_arti,
                    x.modelo,
    ...
    BEGIN
        ...
        ln_linea := 0;
        lv_jason := lv_jason || '[';
        FOR lr_datos IN C_ARTICULOS(Pv_modelo, Pv_marca, Pv_noCia) LOOP
             IF ln_linea > 0 THEN
                lv_jasonaux := lv_jasonaux||',';
             END IF;
            lv_regjson := '{'
                                ...
                                || '",'
                                || '"description":"'
                                || REPLACE(lr_datos.desc_articulo,chr(10),' ')
                                || '",'
                                || '"brand":"'
                                || lr_datos.marca
                                || '",'
                                ...
                                || '",'
                                || '"itemType":"'
                                || lr_datos.tipo_articulo
                                || '",'
                                || '"cost":"'
                                || lr_datos.costo_unitario
                                || '",'
                                || '"unit":"'
                                || lr_datos.unidad
                                || '",'
                                || '"status":"'
                                || REPLACE(lr_datos.estado_arti_activo,chr(10),' ')
                                || '"'
                                || '}';
        ...
        IF ln_linea = 0 THEN
            pv_status := '404';
            pv_mensaje := 'No se encontraron materiales';
        END IF;
        lv_jason := lv_jason || ']';
        RETURN lv_jason;
        
END F_GET_JSON_ARTICULOS_MM;
  --
  --
FUNCTION F_GET_JSON_ARTICULOS_MARCA (           Pv_NoCia       VARCHAR2,
                                       Pv_Status  OUT VARCHAR2,
        ...
        lv_jasonaux      CLOB := NULL;
        lv_regjson       CLOB := NULL;
        CURSOR C_MARCAS(id_code VARCHAR2) IS
        SELECT marc.codigo brandId, 
               marc.descripcion brandDescription 
        FROM NAF47_TNET.marcas marc 
        where no_cia = id_code;
    BEGIN
        pv_status := 'OK';
        pv_mensaje := 'Transaccion exitosa';
        ...
        FOR lr_datos IN C_MARCAS(Pv_NoCia) LOOP
            IF ln_linea > 0 THEN
                lv_jasonaux := lv_jasonaux||',';
             END IF;
            lv_regjson := '{'
                                ...
                                || '",'
                                || '"brandDescription":"'
                                || REPLACE(lr_datos.brandDescription,chr(10),' ')
                                || '"'
                                || '}';
    ...
    --
        IF ln_linea = 0 THEN
            pv_status := '404';
            pv_mensaje := 'No se encontraron Marcas';
        END IF;
        lv_jason := lv_jason || ']';
        RETURN lv_jason;
    END F_GET_JSON_ARTICULOS_MARCA;
  --
  --
...
FUNCTION F_GET_JSON_SUBGRUPO ( Pv_Grupo   VARCHAR2,
  ...
  BEGIN
    ...
    IF Ln_IdActivo IS NOT NULL THEN
      Pv_MensajeError := 'N�mero de serie pertenece a acivo fijo '||Ln_IdActivo||' dado de Baja';
      Lb_SerieValida := FALSE;
      GOTO ET_FIN_VALIDACION;
    END IF;
    --
