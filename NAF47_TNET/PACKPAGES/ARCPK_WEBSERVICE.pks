...
*/
  ...
  */
  /*
   *  Modificacion para version del xml 2.0.0 en comprobantes de retencion electronicos
   *  @author Johnny Xavier Zurita Medranda<jxzurita@telconet.ec>
   *  @version 1.5 14/01/2023 
   */
 ...
 */
  ...
  */
  ...
  /**
  * Documentacion para P_ENVIAR_NOTIFICACION   
  * Procedimiento que envia una notificacion al correo electronico de la empresa con informacion del comprobante erroneo                   
    @author Wilson Chimbo <wchimbo@telconet.ec>
  * @version 1.0 25/11/2022 
  */
  PROCEDURE P_ENVIAR_NOTIFICACION (Pv_No_Cia IN ARCPMD.NO_CIA%TYPE,
                                    Pv_Documento_Id IN DB_COMPROBANTES.INFO_DOCUMENTO.ID_DOCUMENTO%TYPE);                               

end ARCPK_WEBSERVICE;
/

CREATE OR REPLACE package body NAF47_TNET.ARCPK_WEBSERVICE is
...
-- ESQUEMA OFFLINE
    ...
    --jxzurita Query parametrizado Inicio
    TYPE cur_typ IS REF CURSOR;
    c_cursor    CUR_TYP;
    C_query    CLOB;

    ln_no_cia                         NAF47_TNET.arcpmd.no_cia%TYPE;
    lv_no_docu                        NAF47_TNET.arcpmd.no_docu%TYPE;
    lv_rucEmpresa                     NAF47_TNET.arcgmc.id_tributario%TYPE;
    lv_tipoDocumento                  varchar2(2);
    lv_tipoAdmiDocumento              DB_COMPROBANTES.ADMI_TIPO_DOCUMENTO.id_tipo_doc%TYPE;
    ld_fecha_documento                NAF47_TNET.arcpmd.fecha_documento%TYPE;
    ld_fecha_retencion                NAF47_TNET.arcpmd.fecha_retencion%TYPE;
    lv_establecimiento                NAF47_TNET.arcpmd.comp_ret_serie%TYPE;
    lv_ptoEmision                     NAF47_TNET.arcpmd.comp_ret_serie%TYPE;
    lv_secuencial                     NAF47_TNET.arcpmd.comp_ret%TYPE;
    ln_tot_ret                        NAF47_TNET.arcpmd.tot_ret%TYPE;
    ln_nombre_archivo                 NAF47_TNET.arcpmd.nombre_archivo%TYPE;
    lv_usuario_ws                     NAF47_TNET.arcpct.usuario_ws%TYPE;
    lv_clave_ws                       NAF47_TNET.arcpct.clave_ws%TYPE;
    lv_estado_sri                     NAF47_TNET.arcpmd.estado_sri%TYPE;
    ln_numero_envio_sri               NAF47_TNET.arcpmd.numero_envio_sri%TYPE;
    lv_clave_acceso                   NAF47_TNET.arcpmd.clave_acceso%TYPE;
    ln_documento_id                   NAF47_TNET.arcpmd.documento_id%TYPE;
    lv_no_prove                       NAF47_TNET.arcpmp.no_prove%TYPE;
    ln_tipo_id_tributario             NAF47_TNET.arcpmp.tipo_id_tributario%TYPE;
    lv_identificacion                 NAF47_TNET.arcpmp.cedula%TYPE;
    lv_nombre                         NAF47_TNET.arcpmp.nombre%TYPE;
    lv_email1                         NAF47_TNET.arcpmp.email1%TYPE;
    lv_email2                         NAF47_TNET.arcpmp.email2%TYPE;
    lv_direccion                      NAF47_TNET.arcpmp.direccion%TYPE;
    lv_telefono                       NAF47_TNET.arcpmp.telefono%TYPE;
    lv_pais                           NAF47_TNET.arcpmp.pais%TYPE;
    lv_provincia                      NAF47_TNET.arcpmp.provincia%TYPE;
    lv_canton                         NAF47_TNET.arcpmp.canton%TYPE;
    lx_result_xml                     xmltype;
    --jxzurita Query parametrizado Fin
...
-----
  ...
  Lv_NoDocu                    arcpmd.no_docu%type;
    --
BEGIN  
 for i in (select 18 as empresa from dual  union select 10 as empresa from dual ) loop
          begin
                SELECT 
               B.PARAMETRO_clob into C_query
            FROM ge_grupos_parametros A, ge_parametros B
            WHERE A.ID_GRUPO_PARAMETRO = B.ID_GRUPO_PARAMETRO
              AND A.ID_EMPRESA = B.ID_EMPRESA
              AND A.ID_APLICACION = B.ID_APLICACION
              AND A.ID_EMPRESA = i.empresa
              AND A.ID_GRUPO_PARAMETRO = 'COMPRETELECT_SQL'
              AND A.ESTADO = 'A';
         exception
           when others then
             Lv_MessageError := Lv_MessageError || ' No hay query de extraccion parametrizado ';     
             Raise Le_Error;
         end; 
         OPEN c_cursor FOR C_query;
           LOOP
             FETCH c_cursor INTO ln_no_cia, lv_no_docu, lv_rucEmpresa, lv_tipoDocumento, lv_tipoAdmiDocumento, ld_fecha_documento, ld_fecha_retencion,               
            lv_establecimiento, lv_ptoEmision, lv_secuencial, ln_tot_ret, ln_nombre_archivo, lv_usuario_ws, lv_clave_ws, lv_estado_sri,  ln_numero_envio_sri,              
            lv_clave_acceso, ln_documento_id, lv_no_prove, ln_tipo_id_tributario, lv_identificacion, lv_nombre,  lv_email1, lv_email2, lv_direccion,                     
            lv_telefono, lv_pais, lv_provincia,  lv_canton, lx_result_xml;                 
             EXIT WHEN c_cursor%NOTFOUND;

        --    For ret in C_retenciones loop
                -- Para guaradar el referencia de registro a revicsar
                 Lv_NoCia       := ln_no_cia;
                 Lv_NoDocu    := lv_no_docu;
                ...
                --
                Case when  lv_email1 is not null and  lv_email2 is not null then
                       Lv_mails_proveedor := lv_email1 ||';'|| lv_email2;
                     when lv_email1 is not null then
                       Lv_mails_proveedor := lv_email1;
                     when lv_email2 is not null then
                       Lv_mails_proveedor := lv_email2;
                End Case;
                ...
                IF Lv_OrigenDocumento = 'Offline' THEN
                       ...
                       OPEN C_UsuarioExiste(lv_identificacion);
                       FETCH C_UsuarioExiste INTO Ln_IdUsuario;
                        --
                        IF (C_UsuarioExiste%notfound) THEN
                          -- Obtencin de clave para el Usuario  
                         Lv_Password  := DB_FINANCIERO.FNCK_COM_ELECTRONICO.F_GENERA_PASSWD_SHA256 (lv_identificacion);
                          --
                          Ln_IdUsuario                                             :=DB_COMPROBANTES.SEQ_ADMI_USUARIO.NEXTVAL;
                          Lr_CompAdmiUsuario.ID_USUARIO        :=Ln_IdUsuario;
                          Lr_CompAdmiUsuario.LOGIN                    :=lv_identificacion;
                          Lr_CompAdmiUsuario.NOMBRES             :=lv_nombre;
                        ...
                        OPEN c_ParametroEmpresa(ln_no_cia);
                        FETCH c_ParametroEmpresa INTO Ln_IdEmpresa, Ln_AmbienteId;
                        CLOSE c_ParametroEmpresa;

                        OPEN c_wsParametroCiudad(lv_pais,  lv_provincia, lv_canton );
                        ...
                        END IF;
                        --
                        OPEN c_wsParametroTipoIdentif( ln_no_cia,  lv_no_prove );
                        ...
                        Lr_CompInfoDocumento.TIPO_DOC_ID              := lv_tipoAdmiDocumento;
                        Lr_CompInfoDocumento.FORMATO_ID               := 1;
                        Lr_CompInfoDocumento.EMPRESA_ID                := Ln_IdEmpresa;
                        Lr_CompInfoDocumento.NOMBRE                       := ln_nombre_archivo;
                        ...
                        Lr_CompInfoDocumento.PUNTO_EMISION         := lv_ptoEmision;
                        Lr_CompInfoDocumento.VALOR                           := ln_tot_ret;
                        Lr_CompInfoDocumento.ESTADO_DOC_ID          := 10;
                        Lr_CompInfoDocumento.VERSION                        := 2;
                        Lr_CompInfoDocumento.SECUENCIAL                 := lv_secuencial;
                        Lr_CompInfoDocumento.FE_RECIBIDO                := SYSDATE;
                        Lr_CompInfoDocumento.TIPO_IDENTIFICACION_ID    := Ln_IdTipoIdentificacion;
                        Lr_CompInfoDocumento.IDENTIFICACION            := lv_identificacion;
                        Lr_CompInfoDocumento.TIPO_EMISION_ID           := 1;
                        Lr_CompInfoDocumento.USUARIO_ID                    := Ln_IdUsuario;
                        Lr_CompInfoDocumento.LOTEMASIVO_ID             := 0;
                        Lr_CompInfoDocumento.XML_ORIGINAL               := DB_FINANCIERO.FNCK_COM_ELECTRONICO.F_XMLBLOG(lx_result_xml);
                        Lr_CompInfoDocumento.AMBIENTE_ID                  := Ln_AmbienteId;
                        Lr_CompInfoDocumento.FE_EMISION                      := ld_fecha_retencion;
                        ...
                        Lr_CompInfoDocumento.CLAVE_ACCESO              := lv_clave_acceso;
                        Lr_CompInfoDocumento.DOCUMENTO_ID_FINAN:= 0;
                End if;

                IF  ln_numero_envio_sri = 0 THEN  --UPPER(Pv_TipoTransaccion) =  'INSERT'   THEN
                   --
                      DB_FINANCIERO.FNCK_COM_ELECTRONICO.P_INSERT_INFO_DOCUMENTO(Lr_CompInfoDocumento, Lv_MessageError);
                      --
                       IF Lv_MessageError IS NOT NULL  THEN  
                          --
                          BEGIN
                        ...
                        END IF;
                   --
                ELSIF  ln_numero_envio_sri  >  0  THEN  --UPPER(Pv_TipoTransaccion) =  'UPDATE'  THEN
                      --
                      --ACTUALIZO INFO_DOCUMENTO
                      Lr_CompInfoDocumento.FE_ULT_MOD        := SYSDATE;
                      Lr_CompInfoDocumento.USR_ULT_MOD       := TRIM(User);
                      Ln_IdDocumento                         := ln_documento_id;
                      Lr_CompInfoDocumento.ID_DOCUMENTO      := Ln_IdDocumento;
                      DB_FINANCIERO.FNCK_COM_ELECTRONICO_TRAN.P_Update_Info_Documento_NAF(Lr_CompInfoDocumento, Lv_MessageError);
                END IF;
                 ...
                 Else
                      ...
                      UPDATE arcpmd
                              SET estado_sri = '10',    -- EnviadoCompElectTN
                                      ...
                                      tipo_doc_id = lv_tipoAdmiDocumento,
                                      empresa_id = Ln_IdEmpresa
                       WHERE no_cia =  ln_no_cia
                            AND no_docu = lv_no_docu;
  ...
  Begin
    For est in C_RegistroConsulto Loop  -- Registro con estado_sri in ('1','3','10')
        ...
        Elsif  Ln_estado_doc_id = 2  Then  -- (Rechazado,  obtener motivo de rechazo en INFO_MENSAJE)
            ...
            P_ENVIAR_NOTIFICACION (est.no_cia,
                                   est.documento_id);                 
...
Begin
    Lv_ClaveAcceso   :=  DB_FINANCIERO.FNCK_COM_ELECTRONICO.F_GENERATORCLAVE (Pv_FechaDocumento,
...
BEGIN
       ...
       END IF;
        ...
        IF Lv_OrigenDocumento = 'Offline' THEN
               ...
               END IF;
                ...
                Lr_CompInfoDocumento.USUARIO_ID                := Ln_IdUsuario;
                Lr_CompInfoDocumento.LOTEMASIVO_ID             := 0;
                Lr_CompInfoDocumento.XML_ORIGINAL              := DB_FINANCIERO.FNCK_COM_ELECTRONICO.F_XMLBLOG(DOC.result_xml);
                Lr_CompInfoDocumento.AMBIENTE_ID               := Ln_AmbienteId;
  ...
  BEGIN
    FOR est in C_REGISTROCONSULTO LOOP  -- Registro con estado_sri in ('1','3','10')
        ...
        IF Ln_estado_doc_id =2 OR Ln_estado_doc_id= 0 THEN
           P_ENVIAR_NOTIFICACION (est.no_cia,
                                  est.documento_id);
        END IF;
    END LOOP;
  ...
  PROCEDURE P_ENVIAR_NOTIFICACION (Pv_No_Cia IN ARCPMD.NO_CIA%TYPE,
                                  Pv_Documento_Id IN DB_COMPROBANTES.INFO_DOCUMENTO.ID_DOCUMENTO%TYPE) IS

  --VARIABLES PARA CONFIGURACION DE ENVIO DE CORREO
  Lv_ImagenRuta DB_GENERAL.ADMI_PARAMETRO_DET.VALOR1%TYPE:='';
  Lv_correoEmisor DB_GENERAL.ADMI_PARAMETRO_DET.VALOR1%TYPE:='' ;
  Lv_correoReceptor DB_GENERAL.ADMI_PARAMETRO_DET.VALOR1%TYPE:='';
  Lv_correoAsunto DB_GENERAL.ADMI_PARAMETRO_DET.VALOR1%TYPE:='';
  Lv_TipoDocumeto DB_COMPROBANTES.ADMI_TIPO_DOCUMENTO.NOMBRE%TYPE:='';
  --VARIABLES CONTROL DE ERRORES
  Ln_CONTADOR NUMBER := 0;
  Lv_Err_Msg VARCHAR2(1000);

  --CURSOR QUE OBTIENE TODA LA INFORMACION DE ACUERDO AL ID DEL DOCUMENTO
  CURSOR C_Detalle_Notificacion
  (Cn_Documento_Id DB_COMPROBANTES.INFO_DOCUMENTO.ID_DOCUMENTO%TYPE)
  IS
    SELECT A.TIPO_DOC_ID, 
      A.EMPRESA_ID, 
      D.NOMBRE,
      A.ESTABLECIMIENTO ||'-'|| A.PUNTO_EMISION ||'-'|| A.SECUENCIAL AS SERIE, 
      to_char(A.FE_CREACION,'DD-MM-YYYY') AS FECHA, 
      A.IDENTIFICACION,
      C.NOMBRES, 
      B.NOMBRE AS ESTADO
    FROM DB_COMPROBANTES.INFO_DOCUMENTO  A, 
      DB_COMPROBANTES.ADMI_ESTADO_DOCUMENTO B, 
      DB_COMPROBANTES.ADMI_USUARIO C,
      DB_COMPROBANTES.ADMI_TIPO_DOCUMENTO D
    WHERE A.ID_DOCUMENTO = Cn_Documento_Id
      AND A.ESTADO_DOC_ID = B.ID_EST_DOC
      AND A.IDENTIFICACION = C.LOGIN
      AND A.TIPO_DOC_ID = D.ID_TIPO_DOC;

  --CURSOR PARA OBTENCION DE DATOS BASICOS EN EL CASO DE QUE HAYA ALGUNA INCOSISTENCIA
  CURSOR C_Detalle_Notificacion_Simple
  (Cn_Documento_Id DB_COMPROBANTES.INFO_DOCUMENTO.ID_DOCUMENTO%TYPE)
  IS
    SELECT A.TIPO_DOC_ID, 
      A.EMPRESA_ID, 
    ...
    FROM DB_COMPROBANTES.INFO_DOCUMENTO  A, 
      DB_COMPROBANTES.ADMI_ESTADO_DOCUMENTO B,
      DB_COMPROBANTES.ADMI_TIPO_DOCUMENTO C
    WHERE  A.ID_DOCUMENTO = Cn_Documento_Id
      AND A.ESTADO_DOC_ID = B.ID_EST_DOC
      AND A.TIPO_DOC_ID = C.ID_TIPO_DOC;

  --CURSOR QUE OBTIENE PARAMETROS DE CONFIGURACION DE ENVIO DE ACUERDO A LA EMPRESA
  CURSOR C_Parametros
  IS
    SELECT PAD.VALOR1,
    PAD.VALOR2
    FROM DB_GENERAL.ADMI_PARAMETRO_CAB PAC,
      DB_GENERAL.ADMI_PARAMETRO_DET PAD
    WHERE PAC.NOMBRE_PARAMETRO = 'PARAMETROS-EMAIL-ENVIO-NOTIFICACION'
      AND PAC.ID_PARAMETRO  = PAD.PARAMETRO_ID
      AND PAC.ESTADO = 'Activo'
      AND PAD.ESTADO = 'Activo'
      AND PAD.EMPRESA_COD = Pv_No_Cia
      ORDER BY VALOR2;

  --CURSOR QUE OBTIENE EL MENSAJE DEL COMPROBANTE     
  CURSOR C_Mensaje 
  (Cn_Documento_Id DB_COMPROBANTES.INFO_DOCUMENTO.ID_DOCUMENTO%TYPE)
  IS
    SELECT  A.MENSAJE
    FROM DB_COMPROBANTES.INFO_MENSAJE A
    WHERE DOCUMENTO_ID = Cn_Documento_Id
      AND ID_MENSAJE IN ( SELECT MAX(ID_MENSAJE) MENSAJE
                          FROM DB_COMPROBANTES.INFO_MENSAJE
                          WHERE DOCUMENTO_ID = Cn_Documento_Id);

  --VARIABLES DE TIPO CURSOR   
  Lc_DetalleNotificacion C_Detalle_Notificacion%ROWTYPE;
  Lc_DetalleNotificacionSimple C_Detalle_Notificacion_Simple%ROWTYPE;
  Lc_MensajeDocumento C_Mensaje%ROWTYPE;
  Lc_Parametros C_Parametros%ROWTYPE;

  BEGIN

  FOR Lc_Parametros IN C_Parametros LOOP

    IF Lc_Parametros.VALOR2 = 1 THEN
      Lv_ImagenRuta := Lc_Parametros.VALOR1;
    ELSIF Lc_Parametros.VALOR2 = 2 THEN
      Lv_correoEmisor:= Lc_Parametros.VALOR1;
    ELSIF Lc_Parametros.VALOR2 = 3 THEN
      Lv_correoReceptor := Lc_Parametros.VALOR1;
    ELSIF Lc_Parametros.VALOR2 = 4 THEN
      Lv_correoAsunto := Lc_Parametros.VALOR1;
    END IF;
  END LOOP;

  IF (LENGTH(Lv_ImagenRuta) > 0 ) and (LENGTH(Lv_correoEmisor) > 0 ) and (LENGTH(Lv_correoReceptor) > 0) and (LENGTH(Lv_correoAsunto) > 0) THEN

    OPEN C_Detalle_Notificacion(Pv_Documento_Id);

    FETCH  C_Detalle_Notificacion INTO Lc_DetalleNotificacion;

    --CONDICIONAL QUE SE UTILIZA CUANDO NO EXISTE INCOSISTENIAS EN EL COMPROBANTE
    IF LENGTH(Lc_DetalleNotificacion.NOMBRE)>0 THEN
    Ln_Contador := 1;

    OPEN C_Mensaje(Pv_Documento_Id); 
    FETCH C_Mensaje INTO Lc_MensajeDocumento;
    CLOSE C_Mensaje;

      SYS.UTL_MAIL.SEND(SENDER     => Lv_correoEmisor,
                        RECIPIENTS => Lv_correoReceptor,
                        CC         => null,
                        SUBJECT    => Lv_correoAsunto,
                        MIME_TYPE  => 'text/html; charset=ISO-8859-1',
                        MESSAGE    => '
      <div style="background-color:#ffffff">
          <div style="text-align: left;background-color:#ffffff; padding: 10px;">
              <img width="25%" src="'||Lv_ImagenRuta||'" alt="">
              <!--<h2 style="display: inline;">Megadatos</h2>-->
          </div>
          ...
          <div style="background-color: #f4f4f4; padding: 15px;display: block;">
              ...
              <div style="background-color:#ffffff; padding: 15px;text-align:justify;">
                  ...
                  ||'</b> número <b>'||Lc_DetalleNotificacion.SERIE||'</b>,
                      ...
                      documento es <b>'||Lc_DetalleNotificacion.ESTADO||': '||Lc_MensajeDocumento.MENSAJE||'.</b></p>
              </div>
          </div>
      </div>');
    ...
    CLOSE C_Detalle_Notificacion;

    --CONDICIONAL QUE SE UTILIZA EN EL CASO DE QUE EXISTA INCOSISTENCIAS EN EL COMPROBANTE ELECTRONICO
    IF Ln_Contador = 0 THEN
      ...
      OPEN C_Detalle_Notificacion_Simple(Pv_Documento_Id);

      FETCH C_Detalle_Notificacion_Simple INTO Lc_DetalleNotificacionSimple;
          ...
          <div style="background-color: #f4f4f4; padding: 15px;display: block;">
              ...
              <div style="background-color:#ffffff; padding: 15px;text-align: justify;">
                  ...
                  ||'</b> número <b>'||Lc_DetalleNotificacionSimple.SERIE||'</b>,
                      ...
                      sido rechazado, el motivo del porque se ha devuelto el
                      documento es <b>'||Lc_DetalleNotificacionSimple.ESTADO||': '||Lc_MensajeDocumento.MENSAJE||'.</b></p>
              </div>
          </div>
      </div>');
