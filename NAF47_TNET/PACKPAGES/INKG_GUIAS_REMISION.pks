CREATE EDITIONABLE PACKAGE            INKG_GUIAS_REMISION AS 
...
CREATE EDITIONABLE PACKAGE BODY            INKG_GUIAS_REMISION AS
  ...
  */
   ...
   BEGIN
      ...
      Lr_CompInfoDocumento.XML_ORIGINAL           := DB_FINANCIERO.FNCK_COM_ELECTRONICO.F_XMLBLOG(Lc_GuiaRemision.result_xml);
  ...
  Begin
    FOR est in C_GUIA_REMISION LOOP  
        ...
        ELSIF  Ln_EstadoDocId = 2  THEN  -- (Rechazado,  obtener motivo de rechazo en INFO_MENSAJE)
             ...
             NAF47_TNET.ARCPK_WEBSERVICE.P_ENVIAR_NOTIFICACION(est.NO_CIA,
                                                    		 est.documento_id);
