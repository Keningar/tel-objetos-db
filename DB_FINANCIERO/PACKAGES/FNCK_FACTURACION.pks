CREATE EDITIONABLE PACKAGE               FNCK_FACTURACION AS
  ...
  */
   ...
   *
   * @author Alex Arreaga <atarreaga@telconet.ec>
   * @version 1.6 07/04/2021 Se modifica sentencia al where para truncar a dos decimales el precio de venta del servicio en el cursor C_ObtienePuntos.
  ...
  */                                  
   ...
   *
   * @param  NUMBER     IN  Pn_IdDocumento
   * @param  Lrf_Result OUT Prf_Result
   * @param  VARCHAR2   OUT Pv_Status
   * @param  VARCHAR2   OUT Pv_Mensaje
   */                                       
   PROCEDURE P_DESCRI_PRODUCTOS_X_FACTURA(Pn_IdDocumento NUMBER,
                                          Prf_Result      OUT Lrf_Result,
                                          Pv_Status       OUT VARCHAR2,
                                          Pv_Mensaje      OUT VARCHAR2);
  ...
  */      
  PROCEDURE P_FACT_ECDF_CANCEL(Pn_ServicioId      IN NUMBER,
                               Pv_PrefijoEmpresa  IN VARCHAR2,
                               Pv_EmpresaCod      IN VARCHAR2,
                               Pv_UsrCreacion     IN VARCHAR2,
                               Pv_Status          OUT VARCHAR2,
...
CREATE EDITIONABLE PACKAGE BODY               FNCK_FACTURACION AS
  ...
  CURSOR C_ObtienePuntosPorCRS (Cv_PrefijoEmpresa   VARCHAR2,
    ...
    WHERE   ISC.ESTADO = Cv_EstadoServCarac
        ...
        AND NOT EXISTS (SELECT  1 
                  FROM DB_COMERCIAL.INFO_EMPRESA_ROL IERO 
                  INNER JOIN DB_COMERCIAL.ADMI_ROL ARO ON ARO.ID_ROL = IERO.ROL_ID
                  WHERE ARO.DESCRIPCION_ROL = 'Cliente Canal' 
  ...
  PROCEDURE P_FACTURACION_UNICA(Pv_UsrCreacion     IN VARCHAR2,
    ...
    Lv_AdmiParamDetDescrip        DB_GENERAL.ADMI_PARAMETRO_DET.DESCRIPCION%TYPE        := 'BANDERA_FECHA_ACTIVACION';
    ...
    IS
      ...
      AND ISH.FE_CREACION                between TO_TIMESTAMP(TO_CHAR(SYSDATE-1, 'YYYY-MM-DD')||' 22:00:00','RRRR/MM/DD HH24:MI:SS') and TO_TIMESTAMP(TO_CHAR(SYSDATE, 'YYYY-MM-DD')||' 23:59:59','RRRR/MM/DD HH24:MI:SS')
      AND ISE.cantidad                   > Cn_CantidadServicio
      AND ISE.ES_VENTA                   = Cv_EsVentaServicio
      AND TRUNC(ISE.precio_venta,2)      > Cn_PrecioVentaServ
      ...
      --Costo: 3
      CURSOR C_ObtieneParametroDet (Cv_NombreParametro         DB_GENERAL.ADMI_PARAMETRO_CAB.NOMBRE_PARAMETRO%TYPE,
                                    Cv_Modulo                  DB_GENERAL.ADMI_PARAMETRO_CAB.MODULO%TYPE,
                                    Cv_DescParametroDet        DB_GENERAL.ADMI_PARAMETRO_DET.DESCRIPCION%TYPE,
      ...
      IS  
        ...
        AND APD.DESCRIPCION = Cv_DescParametroDet
        AND APD.EMPRESA_COD = Cv_EmpresaCod;

        Lr_ObtieneParamFechaActivacion C_ObtieneParametroDet%ROWTYPE;
    BEGIN

      OPEN C_ObtieneParametroDet(Lv_AdmiParamCabFechaActiv,
                                Lv_ModuloFinanciero,
                                Lv_AdmiParamDetDescrip,
                                Lv_EstadoActivo,
                                Pv_EmpresaCod);
      FETCH C_ObtieneParametroDet
        INTO Lr_ObtieneParamFechaActivacion;
      CLOSE C_ObtieneParametroDet;
  ...
  CURSOR C_GET_SERV_FACTURAR (  Cn_IdServicio      DB_COMERCIAL.INFO_SERVICIO.ID_SERVICIO%TYPE) IS
     ...
     AND   ISH.ID_SERVICIO_HISTORIAL = (SELECT MAX(ISH2.ID_SERVICIO_HISTORIAL) 
      ...
      LOOP
        ...
        LOOP
          ...
          IF Ln_NumMesesActivo < Ln_NumMesesTotal THEN
            ...
            END IF;
            OPEN C_EsClienteAFacturar(Lr_InfoDocumentoFinancieroDet.SERVICIO_ID);
            ...
            IF Ln_BanderaClienteFacturar='N' THEN
              Lv_MsnError := 'Rol de Cliente no debe facturar';
              RAISE Le_Error;
            END IF;
  ...
  BEGIN
    ...
    Lv_Query := ' SELECT DISTINCT LISTAGG(DESCRIPCION_PRODUCTO, '','') 
                         WITHIN GROUP (ORDER BY DESCRIPCION_PRODUCTO) over (partition by GRUPO) DESCRIPCION_PRODUCTO
                         FROM (SELECT 
                                (CASE 
                                 ...
                                 END)DESCRIPCION_PRODUCTO, ''1'' AS GRUPO 
                               FROM DB_FINANCIERO.INFO_DOCUMENTO_FINANCIERO_DET IDFD 
                               WHERE IDFD.DOCUMENTO_ID='||Pn_IdDocumento||') tabla ';
  ...
  WHEN OTHERS THEN
    ...
    Pv_Mensaje := DBMS_UTILITY.FORMAT_ERROR_STACK||'-'||DBMS_UTILITY.format_call_stack||chr(13);
    Pv_Status  := 'ERROR';
    DB_FINANCIERO.FNCK_COM_ELECTRONICO_TRAN.INSERT_ERROR('Error en proceso P_DESCRI_PRODUCTOS_X_FACTURA', 'FNCK_FACTURACION.P_DESCRI_PRODUCTOS_X_FACTURA', Pv_Mensaje);
  
  
  END P_DESCRI_PRODUCTOS_X_FACTURA;
...
PROCEDURE P_FACT_ECDF_CANCEL(Pn_ServicioId      IN NUMBER,
  ...
  Lv_ModuloFinanciero           VARCHAR2(20)  := 'FINANCIERO';
  Lv_ParametroPeriodoRenovacion VARCHAR2(50)  := 'PRODUCTO_FECHA_MINIMA_SUSCRIPCION';
  Lv_DescripParamDetMesMinimo   VARCHAR2(50)  := 'MESES_MINIMOS';
  Lv_NombreProcedimiento        VARCHAR2(50)  := 'FNCK_FACTURACION_MENSUAL.P_FACT_ECDF_CANCEL';
  Lv_IpLocal                    VARCHAR2(15)  := '127.0.0.1';
  Lv_DescripCaractFeActivacion  VARCHAR2(20)  := 'FECHA_ACTIVACION';
  Lv_DescripCaractFeFinal       VARCHAR2(30)  := 'FECHA_MINIMA_SUSCRIPCION';
  Lv_ParamCabDescripFactura     VARCHAR2(35)  := 'FACTURACION_SERV_ADICIONAL_ECDF';
  ...
  SELECT 
        ...
        WHERE is1.estado= Lv_EstadoCancelado
        and IS1.ID_SERVICIO = Cn_IdServicio
        AND ac.descripcion_caracteristica= Lv_DescripCaractFeActivacion;

  --Cursor que obtiene la fecha final
  CURSOR  C_GET_FE_FINPERIODO(Cn_IdServicio NUMBER) IS
  SELECT 
        TO_CHAR(TO_DATE(ISPC.VALOR, 'YYYY-MM-DD HH24:MI:SS'),'DD MONTH YYYY','NLS_DATE_LANGUAGE=SPANISH')
        FROM DB_COMERCIAL.admi_caracteristica AC
        LEFT JOIN DB_COMERCIAL.admi_producto_caracteristica APC ON apc.caracteristica_id = ac.id_caracteristica
        LEFT JOIN DB_COMERCIAL.info_servicio_prod_caract ISPC ON ispc.producto_caracterisitica_id = apc.id_producto_caracterisitica
        LEFT JOIN DB_COMERCIAL.info_servicio IS1 ON is1.id_servicio = ispc.servicio_id
        WHERE is1.estado= Lv_EstadoCancelado
        and IS1.ID_SERVICIO = Cn_IdServicio
        AND ac.descripcion_caracteristica= Lv_DescripCaractFeFinal;
        
  --Cursor que obtiene los parametros 
  CURSOR C_GET_PARAMETROS(Cv_EmpresaCod VARCHAR2, Cv_NombreParametro VARCHAR2, Cv_Modulo VARCHAR2, Cv_Estado VARCHAR2, Cv_DescripcionDet VARCHAR2) IS
    SELECT DET.VALOR1,
           DET.VALOR2,
           DET.VALOR3,
           DET.VALOR4
   ...
   --Cursor que obtiene el punto a facturar.
   CURSOR C_GET_PTOS_FACTURAR(Cv_PrefijoEmpresa VARCHAR2,Cv_DescripcionProducto VARCHAR2, Cn_IdServicio NUMBER) IS
     SELECT DISTINCT ISE.PUNTO_FACTURACION_ID,IPER.OFICINA_ID
     FROM DB_COMERCIAL.INFO_SERVICIO            ISE
     JOIN DB_COMERCIAL.INFO_PUNTO               IPT  ON IPT.ID_PUNTO        = ISE.PUNTO_FACTURACION_ID             
...
--Cursor que obtiene la numeracion de la facturas del sri
CURSOR C_GET_NUMERACION (Cv_PrefijoEmpresa VARCHAR2, Cv_OficinaId VARCHAR2, Cv_CodigoNumeracion VARCHAR2) IS
        SELECT AN.*
                FROM DB_FINANCIERO.ADMI_NUMERACION AN,
                     DB_FINANCIERO.INFO_OFICINA_GRUPO IOG,
                     DB_FINANCIERO.INFO_EMPRESA_GRUPO IEG
                WHERE AN.EMPRESA_ID = IEG.COD_EMPRESA
                  AND AN.OFICINA_ID = IOG.ID_OFICINA
                  AND IEG.PREFIJO   = NVL( Cv_PrefijoEmpresa, IEG.PREFIJO)
                  AND IEG.ESTADO    = Lv_EstadoActivo
                  AND IOG.ESTADO    = Lv_EstadoActivo
                  AND IOG.ES_MATRIZ = NVL( Lv_ValorS, IOG.ES_MATRIZ)
                  AND IOG.ES_OFICINA_FACTURACION = NVL(Lv_ValorS, IOG.ES_OFICINA_FACTURACION)
                  AND IOG.ID_OFICINA = NVL(Cv_OficinaId, IOG.ID_OFICINA)
  ...
  Lc_ParametrosCm               C_GET_PARAMETROS%ROWTYPE;
  Lc_ParametrosDescripFact      C_GET_PARAMETROS%ROWTYPE;
  Lc_FechaActivacion            C_FECHA_ACTIVACION%ROWTYPE;
    --Variables de la numeracion
  Lc_AdmiNumeracion             C_GET_NUMERACION%ROWTYPE;

  BEGIN

    --Se obtiene el numero de meses de suscripcion del producto 
    IF C_GET_PARAMETROS%ISOPEN THEN
      CLOSE C_GET_PARAMETROS;
    END IF;

    OPEN C_GET_PARAMETROS(Pv_EmpresaCod,Lv_ParametroPeriodoRenovacion,Lv_ModuloTecnico,Lv_EstadoActivo, Lv_DescripParamDetMesMinimo);
     FETCH C_GET_PARAMETROS
        INTO Lc_ParametrosCm;
    CLOSE C_GET_PARAMETROS;
    ...
    OPEN C_GET_PARAMETROS(Pv_EmpresaCod,Lv_ParamCabDescripFactura,Lv_ModuloFinanciero,Lv_EstadoActivo, Lv_ParamDetDescripFactura);
     FETCH C_GET_PARAMETROS
        INTO Lc_ParametrosDescripFact;
    CLOSE C_GET_PARAMETROS;
    
    IF TRIM(Lc_ParametrosDescripFact.VALOR3) IS NOT NULL THEN
       Lv_DescripcionFacturaDet := Lc_ParametrosDescripFact.VALOR3 || ' ' || Ln_NumMesesTotal || ' MESES';
    END IF;
    ...
    SELECT PORCENTAJE_IMPUESTO INTO Ln_Porcentaje
     ...
     WHERE TIPO_IMPUESTO LIKE 'IVA%'
      ...
      --Se obtienen los datos de la empresa
      FNCK_FACTURACION_MENSUAL.GET_PREFIJO_OFICINA(Pv_EmpresaCod,Lv_PrefijoEmpresa,Ln_IdOficina);

      FOR Lr_PuntosAFacturar IN C_GET_PTOS_FACTURAR(Lv_PrefijoEmpresa,
                                                    Lv_DescripcionProducto,
                                                    Pn_ServicioId)
      LOOP
        BEGIN
        ...
        --Se obtiene la fecha final de suscripcion
          IF C_GET_FE_FINPERIODO%ISOPEN THEN
            CLOSE C_GET_FE_FINPERIODO;
          END IF;

          OPEN C_GET_FE_FINPERIODO(Lr_Servicios.ID_SERVICIO);
           FETCH C_GET_FE_FINPERIODO
              INTO Lv_FechaFinPeriodo;
          CLOSE C_GET_FE_FINPERIODO;
          
        --Se obtiene valores a facturar por servicio
            IF C_GET_VALOR_FACT_SERVICIO%ISOPEN THEN
                CLOSE C_GET_VALOR_FACT_SERVICIO;
              END IF;
        
              OPEN C_GET_VALOR_FACT_SERVICIO(TO_DATE(Lc_FechaActivacion.FECHA_COMPLETA),Lr_Servicios.ID_SERVICIO,Lv_DescripcionProducto);
               FETCH C_GET_VALOR_FACT_SERVICIO
               INTO  Lc_GetValorFactxServicio;
              CLOSE C_GET_VALOR_FACT_SERVICIO;
        
        --Se obtiene el numero de meses con el servicio Activo
          SELECT ROUND(MONTHS_BETWEEN(SYSDATE,Lc_FechaActivacion.FECHA_COMPLETA),1)
          INTO Ln_NumMesesActivo
          FROM DUAL;
          ...
          IF Ln_NumMesesActivo < Ln_NumMesesTotal THEN
            ...
            IF Ln_ContServicios = 0 THEN
                ...
                Lr_InfoDocumentoFinancieroHis.USR_CREACION          := Pv_UsrCreacion;
                Lr_InfoDocumentoFinancieroHis.ESTADO                := Lv_EstadoPendiente;
                Lr_InfoDocumentoFinancieroHis.OBSERVACION           := 'Se crea la factura por cancelacion de servicio '|| Lv_DescripcionProducto;
                FNCK_TRANSACTION.INSERT_INFO_DOC_FINANCIERO_HST(Lr_InfoDocumentoFinancieroHis,Lv_MsnError);
                IF Lv_MsnError IS NOT NULL THEN
                    RAISE Le_Error;
                END IF;                  
            ...
            --Se calcula el valor a facturar
            Ln_PrecioVentaFacProDetalle:= 0;
            Ln_PrecioVentaFacProDetalle:= ROUND((NVL(Lr_Servicios.precio_venta,0) * Ln_NumMesesTotal),2)-ROUND(NVL(Lc_GetValorFactxServicio.VALOR,0),2);

            FNCK_FACTURACION_MENSUAL.GET_SOL_DESCT_UNICO(Lr_Servicios.id_servicio,Ln_IdDetalleSolicitud,Ln_PorcentajeDescuento);
            
            --Calculo de descuento, en caso de aplicar
            IF Ln_PorcentajeDescuento IS NOT NULL AND Ln_PorcentajeDescuento>0 THEN
              FNCK_FACTURACION_MENSUAL.UPD_SOL_DESCT_UNICO(Ln_IdDetalleSolicitud);
              Ln_DescuentoFacProDetalle := ROUND((Ln_PrecioVentaFacProDetalle *Ln_PorcentajeDescuento)/100,2);
            --Verifico si posee descuento fijo por porcentaje o valor; ya que este es el mandatorio  
        ...
        END LOOP;
            ...
            --Actualizo la numeracion SRI y el estado 
            IF Ln_ValorTotal >0 THEN
              --Debo recorrer la numeracion obtenida
                IF C_GET_NUMERACION%ISOPEN THEN
                  CLOSE C_GET_NUMERACION;
                END IF;
                
                OPEN C_GET_NUMERACION(Lv_PrefijoEmpresa,Ln_IdOficina,Lv_CodigoNumeracion);
                    FETCH C_GET_NUMERACION
                    INTO Lc_AdmiNumeracion;
                    EXIT WHEN C_GET_NUMERACION%notfound;
                    Lv_Secuencia :=LPAD(Lc_AdmiNumeracion.SECUENCIA,9,'0');
                    Lv_Numeracion:=Lc_AdmiNumeracion.NUMERACION_UNO || '-'||Lc_AdmiNumeracion.NUMERACION_DOS||'-'||Lv_Secuencia;
                --Cierro la numeracion
                CLOSE C_GET_NUMERACION;
              ...
              --Incremento la numeracion
              Lc_AdmiNumeracion.SECUENCIA:=Lc_AdmiNumeracion.SECUENCIA+1;
              FNCK_TRANSACTION.UPDATE_ADMI_NUMERACION(Lc_AdmiNumeracion.ID_NUMERACION,Lc_AdmiNumeracion,Lv_MsnError);
  ...
  AS
    ...
    Lv_ParametroPeriodoRenovacion VARCHAR2(50)  := 'PRODUCTO_FECHA_MINIMA_SUSCRIPCION';
    Lv_DescripParamDetMesMinimo   VARCHAR2(50)  := 'MESES_MINIMOS';
    Lv_NombreProcedimiento        VARCHAR2(200)  := 'FNCK_FACTURACION_MENSUAL.P_FACT_CANCEL_SERV_ADICIONAL';
    Lv_IpLocal                    VARCHAR2(15)  := '127.0.0.1';
    Lv_DescripCaractFeActivacion  VARCHAR2(20)  := 'FECHA_ACTIVACION';
    Lv_DescripCaractFeFinal       VARCHAR2(30)  := 'FECHA_MINIMA_SUSCRIPCION';
     Lv_ParamCabDescripFactura     VARCHAR2(35)  := 'FACTURACION_SERV_ADICIONAL';
    Lv_ParamDetDescripFactura     VARCHAR2(30)  := 'DESCRIPCION_FACTURA';
    Lv_Mensaje                    VARCHAR2(4000);
    ...
    --Cursor que obtiene la fecha final
    CURSOR  C_GET_FE_FINPERIODO(Cn_IdServicio NUMBER) 
    IS
      SELECT TO_CHAR(TO_DATE(ISPC.VALOR, 'YYYY-MM-DD HH24:MI:SS'),'DD MONTH YYYY','NLS_DATE_LANGUAGE=SPANISH')
      FROM DB_COMERCIAL.admi_caracteristica AC
      LEFT JOIN DB_COMERCIAL.admi_producto_caracteristica APC ON apc.caracteristica_id = ac.id_caracteristica
      LEFT JOIN DB_COMERCIAL.info_servicio_prod_caract ISPC ON ispc.producto_caracterisitica_id = apc.id_producto_caracterisitica
      LEFT JOIN DB_COMERCIAL.info_servicio IS1 ON is1.id_servicio = ispc.servicio_id
    ...
    --Cursor que obtiene los parametros 
    CURSOR C_GET_PARAMETROS(Cv_EmpresaCod VARCHAR2, Cv_NombreParametro VARCHAR2, 
                            Cv_Modulo VARCHAR2, 
                            Cv_Estado VARCHAR2, 
                            Cv_DescripcionDet VARCHAR2,
    ...
    IS
      ...
      WHERE   CAB.ESTADO      =  Cv_Estado
      AND   DET.ESTADO           =  Cv_Estado
      AND   CAB.MODULO           =  Cv_Modulo
      AND   DET.EMPRESA_COD      =  Cv_EmpresaCod
      AND   CAB.NOMBRE_PARAMETRO =  Cv_NombreParametro
      AND   DET.DESCRIPCION      =  Cv_DescripcionDet
      AND   DET.VALOR1           =  Cv_NombreTecnico;
    ...
    CURSOR C_GET_DESC_PRODUCTO(Cn_ServicioId NUMBER) IS
      ...
      INNER JOIN DB_COMERCIAL.INFO_SERVICIO INS ON INS.PRODUCTO_ID = ADP.ID_PRODUCTO
      WHERE INS.ID_SERVICIO = Cn_ServicioId;
    ...
    Ln_ValorTotal                 INFO_DOCUMENTO_FINANCIERO_CAB.VALOR_TOTAL%TYPE;
    Lv_DescripcionFacturaDet      INFO_DOCUMENTO_FINANCIERO_DET.OBSERVACIONES_FACTURA_DETALLE%TYPE := '';
    Ln_ValorServicioCancelFact    DB_FINANCIERO.INFO_DOCUMENTO_FINANCIERO_DET.PRECIO_VENTA_FACPRO_DETALLE%TYPE  := 0;
    Lr_ParametrosCm               C_GET_PARAMETROS%ROWTYPE;
    Lr_ParametrosDescripFact      C_GET_PARAMETROS%ROWTYPE;
    Lr_FechaActivacion            C_FECHA_ACTIVACION%ROWTYPE;
    Lrf_Numeracion                FNKG_TYPES.Lrf_AdmiNumeracion;
    Lr_AdmiNumeracion             FNKG_TYPES.Lr_AdmiNumeracion;
    Lr_InfoServicioAFacturar      C_GET_SERV_FACTURAR%ROWTYPE;
  ...
  BEGIN
    ...
    FNCK_FACTURACION_MENSUAL.GET_PREFIJO_OFICINA(Pv_EmpresaCod,Lv_PrefijoEmpresa,Ln_IdOficina);
    IF C_GET_SERV_FACTURAR%ISOPEN THEN
      CLOSE C_GET_SERV_FACTURAR;
    END IF;

    OPEN C_GET_SERV_FACTURAR(Pn_IdServicio);
    FETCH C_GET_SERV_FACTURAR
    INTO  Lr_InfoServicioAFacturar;

    IF C_GET_SERV_FACTURAR%FOUND THEN
      Lv_NombreTecnicoProd := Lr_InfoServicioAFacturar.NOMBRE_TECNICO;
      --Se obtiene la fecha de activacion del servicio
      IF C_FECHA_ACTIVACION%ISOPEN THEN
        CLOSE C_FECHA_ACTIVACION;
      END IF;

      OPEN C_FECHA_ACTIVACION(Lr_InfoServicioAFacturar.ID_SERVICIO);
      FETCH C_FECHA_ACTIVACION
      INTO Lr_FechaActivacion;
      CLOSE C_FECHA_ACTIVACION;
      ...
      OPEN C_GET_PARAMETROS(Pv_EmpresaCod,Lv_ParametroPeriodoRenovacion,Lv_ModuloTecnico,Lv_EstadoActivo, Lv_DescripParamDetMesMinimo, Lv_NombreTecnicoProd);
       FETCH C_GET_PARAMETROS INTO Lr_ParametrosCm;
      CLOSE C_GET_PARAMETROS;
      ...
      OPEN C_GET_PARAMETROS(Pv_EmpresaCod,Lv_ParamCabDescripFactura,Lv_ModuloFinanciero,Lv_EstadoActivo, Lv_ParamDetDescripFactura, Lv_NombreTecnicoProd);
       FETCH C_GET_PARAMETROS
          INTO Lr_ParametrosDescripFact;
      CLOSE C_GET_PARAMETROS;
      
      IF TRIM(Lr_ParametrosDescripFact.VALOR3) IS NOT NULL THEN
         Lv_DescripcionFacturaDet := Lr_ParametrosDescripFact.VALOR3 || ' ' || Ln_NumMesesTotal || ' MESES';
      END IF;
      ...
      OPEN C_GET_FE_FINPERIODO(Lr_InfoServicioAFacturar.ID_SERVICIO);
      FETCH C_GET_FE_FINPERIODO
      INTO Lv_FechaFinPeriodo;
      CLOSE C_GET_FE_FINPERIODO;

  --Se obtiene descripcion del producto
      IF C_GET_DESC_PRODUCTO%ISOPEN THEN
        CLOSE C_GET_DESC_PRODUCTO;
      END IF;

      OPEN C_GET_DESC_PRODUCTO(Pn_IdServicio);
        FETCH C_GET_DESC_PRODUCTO
          INTO Lv_DescripcionProducto;
      CLOSE C_GET_DESC_PRODUCTO;

      --Se calcula el valor a facturar
      Ln_ValorServicioCancelFact := DB_FINANCIERO.FNCK_CANCELACION_VOL.F_GET_TOTAL_FACTURADO(Lv_DescripcionProducto,
                                    Lr_InfoServicioAFacturar.ID_SERVICIO, Lr_FechaActivacion.FECHA_COMPLETA);
      Ln_PrecioVentaFacProDetalle:= 0;
      Ln_PrecioVentaFacProDetalle:= ROUND((NVL(Lr_InfoServicioAFacturar.cantidad,0) * NVL(Lr_InfoServicioAFacturar.precio_venta,0) 
                                    * NVL(Ln_NumMesesTotal,0)),2)-ROUND(NVL(Ln_ValorServicioCancelFact,0),2);

      --Se obtiene el numero de meses con el servicio Activo
      SELECT ROUND(MONTHS_BETWEEN(SYSDATE,Lr_FechaActivacion.FECHA_COMPLETA),1)
      INTO Ln_NumMesesActivo
      FROM DUAL;
      
      Ln_NumMesesActivo := TRUNC(Ln_NumMesesActivo,0);

      IF Ln_NumMesesActivo < Ln_NumMesesTotal AND Ln_PrecioVentaFacProDetalle > 0 THEN
        ...
        Lr_InfoDocumentoFinancieroCab                       := NULL;
        Lr_InfoDocumentoFinancieroCab.ID_DOCUMENTO          := SEQ_INFO_DOC_FINANCIERO_CAB.NEXTVAL;
        Lr_InfoDocumentoFinancieroCab.OFICINA_ID            := Lr_InfoServicioAFacturar.OFICINA_ID;
        Lr_InfoDocumentoFinancieroCab.PUNTO_ID              := Lr_InfoServicioAFacturar.PUNTO_FACTURACION_ID;
        Lr_InfoDocumentoFinancieroCab.TIPO_DOCUMENTO_ID     := 1;
        Lr_InfoDocumentoFinancieroCab.ES_AUTOMATICA         := Lv_ValorS;
        Lr_InfoDocumentoFinancieroCab.PRORRATEO             := Lv_ValorN;
        Lr_InfoDocumentoFinancieroCab.REACTIVACION          := Lv_ValorN;
        ...
        FNCK_FACTURACION_MENSUAL.GET_SOL_DESCT_UNICO(Lr_InfoServicioAFacturar.id_servicio,Ln_IdDetalleSolicitud,Ln_PorcentajeDescuento);
        ...
        IF Ln_PorcentajeDescuento IS NOT NULL AND Ln_PorcentajeDescuento>0 THEN
          ...
          Ln_DescuentoFacProDetalle := ROUND((Ln_PrecioVentaFacProDetalle *Ln_PorcentajeDescuento)/100,2);
        --Verifico si posee descuento fijo por porcentaje o valor; ya que este es el mandatorio  
        ELSIF Lr_InfoServicioAFacturar.porcentaje_descuento>0 THEN
          Ln_DescuentoFacProDetalle := ROUND((Ln_PrecioVentaFacProDetalle*Lr_InfoServicioAFacturar.porcentaje_descuento)/100,2);
        ELSIF Lr_InfoServicioAFacturar.valor_descuento  >0 THEN
          Ln_DescuentoFacProDetalle := ROUND(Lr_InfoServicioAFacturar.valor_descuento,2); 
        ELSE  
          Ln_DescuentoFacProDetalle := 0;
        END IF;
        ...
        Lr_InfoDocumentoFinancieroDet.ID_DOC_DETALLE             :=SEQ_INFO_DOC_FINANCIERO_DET.NEXTVAL;
        Lr_InfoDocumentoFinancieroDet.DOCUMENTO_ID               :=Lr_InfoDocumentoFinancieroCab.ID_DOCUMENTO;
        Lr_InfoDocumentoFinancieroDet.PUNTO_ID                   :=Lr_InfoServicioAFacturar.punto_id;
        Lr_InfoDocumentoFinancieroDet.PLAN_ID                    :=Lr_InfoServicioAFacturar.plan_id;
        Lr_InfoDocumentoFinancieroDet.CANTIDAD                   :=Lr_InfoServicioAFacturar.cantidad;
        Lr_InfoDocumentoFinancieroDet.PRECIO_VENTA_FACPRO_DETALLE:=ROUND(Ln_PrecioVentaFacProDetalle,2);
        Lr_InfoDocumentoFinancieroDet.PORCETANJE_DESCUENTO_FACPRO:=Ln_PorcentajeDescuento;
        Lr_InfoDocumentoFinancieroDet.DESCUENTO_FACPRO_DETALLE   :=Ln_DescuentoFacProDetalle;
        Lr_InfoDocumentoFinancieroDet.VALOR_FACPRO_DETALLE       :=ROUND(Ln_PrecioVentaFacProDetalle,2);
        ...
        Lr_InfoDocumentoFinancieroDet.FE_CREACION                :=SYSDATE;
        Lr_InfoDocumentoFinancieroDet.USR_CREACION               :=Pv_UsrCreacion;
        Lr_InfoDocumentoFinancieroDet.PRODUCTO_ID                :=Lr_InfoServicioAFacturar.producto_id;
        Lr_InfoDocumentoFinancieroDet.SERVICIO_ID                :=Lr_InfoServicioAFacturar.id_servicio;
        Lr_InfoDocumentoFinancieroDet.OBSERVACIONES_FACTURA_DETALLE  := Lv_DescripcionFacturaDet;
        ...
        Ln_ValorTotal          := NVL( NVL(Ln_Subtotal, 0) - NVL(Ln_SubtotalDescuento, 2) - NVL(Ln_DescuentoCompensacion, 0) 
                                  + NVL(Ln_SubtotalConImpuesto, 0), 0);
        ...
        --Actualizo la numeracion SRI y el estado 
        IF Ln_ValorTotal >0 THEN
          Lrf_Numeracion:=FNCK_CONSULTS.F_GET_NUMERACION(Lv_PrefijoEmpresa,Lv_ValorS,Lv_ValorS,Ln_IdOficina,Lv_CodigoNumeracion);
          FETCH Lrf_Numeracion INTO Lr_AdmiNumeracion;
          Lv_Secuencia :=LPAD(Lr_AdmiNumeracion.SECUENCIA,9,'0');
          Lv_Numeracion:=Lr_AdmiNumeracion.NUMERACION_UNO || '-'||Lr_AdmiNumeracion.NUMERACION_DOS||'-'||Lv_Secuencia;
          --Cierro la numeracion
  ...
  EXCEPTION
    WHEN Le_Error THEN
      ...
      BEGIN
        DB_SOPORTE.SPKG_INFO_TAREA.P_CREA_ACTIVIDAD_PARAMETRIZADA(  'PARAMETROS_ASOCIADOS_A_SERVICIOS_MD',
                                                                    'FACT_CANCEL_SERVICIO_ADICIONAL_' || Lv_NombreTecnicoProd,
    ...
    WHEN OTHERS THEN
      ...
      BEGIN
        DB_SOPORTE.SPKG_INFO_TAREA.P_CREA_ACTIVIDAD_PARAMETRIZADA(  'PARAMETROS_ASOCIADOS_A_SERVICIOS_MD',
                                                                    ...
                                                                    Ln_IdDetalleTarea);
      END;
  END P_FACT_CANCEL_SERV_ADICIONAL;
