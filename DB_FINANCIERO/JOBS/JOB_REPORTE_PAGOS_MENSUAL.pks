BEGIN 
dbms_scheduler.create_job('"JOB_REPORTE_PAGOS_MENSUAL"',
...
BEGIN
  ...
  LV_FECCREADESD    := TO_CHAR(TRUNC(ADD_MONTHS(SYSDATE,-1),''MM''),''DD/MM/YYYY'');
  LV_FECCREAHAST    := TO_CHAR(TRUNC(SYSDATE,''MM''),''DD/MM/YYYY'');
...
start_date=>TO_TIMESTAMP_TZ('02-AUG-2017 02.22.15.754419000 AM AMERICA/GUAYAQUIL','DD-MON-RRRR HH.MI.SSXFF AM TZR','NLS_DATE_LANGUAGE=english'), repeat_interval=> 
'FREQ=MONTHLY;BYMONTHDAY=1;BYHOUR=12;BYMINUTE=30;BYSECOND=0'
, end_date=>NULL,
job_class=>'"DEFAULT_JOB_CLASS"', enabled=>FALSE, auto_drop=>FALSE,comments=>
'REPORTE PARA EL ENVIO AUTOMATICO MENSUAL DEL REPORTE DE PAGOS'
);
sys.dbms_scheduler.set_attribute('"JOB_REPORTE_PAGOS_MENSUAL"','NLS_ENV','NLS_LANGUAGE=''AMERICAN'' NLS_TERRITORY=''AMERICA'' NLS_CURRENCY=''$'' NLS_ISO_CURRENCY=''AMERICA'' NLS_NUMERIC_CHARACTERS=''.,'' NLS_CALENDAR=''GREGORIAN'' NLS_DATE_FORMAT=''DD-MON-RR'' NLS_DATE_LANGUAGE=''AMERICAN'' NLS_SORT=''BINARY'' NLS_TIME_FORMAT=''HH.MI.SSXFF AM'' NLS_TIMESTAMP_FORMAT=''DD-MON-RR HH.MI.SSXFF AM'' NLS_TIME_TZ_FORMAT=''HH.MI.SSXFF AM TZR'' NLS_TIMESTAMP_TZ_FORMAT=''DD-MON-RR HH.MI.SSXFF AM TZR'' NLS_DUAL_CURRENCY=''$'' NLS_COMP=''BINARY'' NLS_LENGTH_SEMANTICS=''BYTE'' NLS_NCHAR_CONV_EXCP=''FALSE''');
dbms_scheduler.set_attribute('"JOB_REPORTE_PAGOS_MENSUAL"','logging_level',DBMS_SCHEDULER.LOGGING_FULL);
dbms_scheduler.enable('"JOB_REPORTE_PAGOS_MENSUAL"');
COMMIT; 
END; 
/ 