BEGIN 
dbms_scheduler.create_job('"JOB_FACTURACION_MASIVA_MD"',
...
''telcos_job_fact'',SYSDATE, NVL(SYS_CONTEXT(''USERENV'',''IP_ADDRESS''), Lv_IpCreacion));
 ...
 ''Inicio de ejecucion P_OBTIENE_GRUPOS_PROC_PIERDE por Pv_TipoProceso: NUEVO'',''telcos_job_fact'',SYSDATE, 
 NVL(SYS_CONTEXT(''USERENV'',''IP_ADDRESS''), Lv_IpCreacion));
  ...
  DB_GENERAL.GNRLPCK_UTIL.INSERT_ERROR(''Telcos+'',''CMKG_PROMOCIONES.P_OBTIENE_GRUPOS_PROC_PIERDE'', 
 ''Inicio de ejecucion P_OBTIENE_GRUPOS_PROC_PIERDE por Pv_TipoProceso: EXISTENTE'',''telcos_job_fact'',SYSDATE,
 NVL(SYS_CONTEXT(''USERENV'',''IP_ADDRESS''), Lv_IpCreacion));
...
''Inicio de ejecucion P_OBTIENE_GRUPOS_PROC_MAPEO'',''telcos_job_fact'',SYSDATE, NVL(SYS_CONTEXT(''USERENV'',''IP_ADDRESS''), Lv_IpCreacion)); 
...
''Inicio de ejecucion P_APLICA_PROMOCION'',''telcos_job_fact'',SYSDATE, NVL(SYS_CONTEXT(''USERENV'',''IP_ADDRESS''), Lv_IpCreacion));
  ...
  DB_GENERAL.GNRLPCK_UTIL.INSERT_ERROR(''Telcos+'',''FNCK_FACTURACION_MENSUAL.P_FACTURACION_MENSUAL'', 
''Inicio de ejecucion P_FACTURACION_MENSUAL'',''telcos_job_fact'',SYSDATE, NVL(SYS_CONTEXT(''USERENV'',''IP_ADDRESS''), Lv_IpCreacion)); 
  DB_FINANCIERO.FNCK_FACTURACION_MENSUAL.P_FACTURACION_MENSUAL (PN_EMPRESACOD  => PN_EMPRESACOD,
                                         PV_DESCRIPCIONIMPUESTO => PV_DESCRIPCIONIMPUESTO, PV_TIPOFACTURACION  => PV_TIPOFACTURACION);
  DB_GENERAL.GNRLPCK_UTIL.INSERT_ERROR(''Telcos+'',''FNCK_FACTURACION_MENSUAL.P_FACTURACION_MENSUAL'', 
''Fin de ejecucion P_FACTURACION_MENSUAL'',''telcos_job_fact'',SYSDATE, NVL(SYS_CONTEXT(''USERENV'',''IP_ADDRESS''), Lv_IpCreacion));
END;'
, number_of_arguments=>0,
start_date=>TO_TIMESTAMP_TZ('16-MAY-2023 12.19.54.306552000 PM AMERICA/GUAYAQUIL','DD-MON-RRRR HH.MI.SSXFF AM TZR','NLS_DATE_LANGUAGE=english'), repeat_interval=> 
'FREQ=DAILY;BYHOUR=0;BYMINUTE=0;BYSECOND=0'
, end_date=>NULL,
job_class=>'"DEFAULT_JOB_CLASS"', enabled=>FALSE, auto_drop=>FALSE,comments=>
'Proceso para ejecutar la facturacion mensual de MEGADATOS'
);
sys.dbms_scheduler.set_attribute('"JOB_FACTURACION_MASIVA_MD"','NLS_ENV','NLS_LANGUAGE=''AMERICAN'' NLS_TERRITORY=''AMERICA'' NLS_CURRENCY=''$'' NLS_ISO_CURRENCY=''AMERICA'' NLS_NUMERIC_CHARACTERS=''.,'' NLS_CALENDAR=''GREGORIAN'' NLS_DATE_FORMAT=''DD-MON-RR'' NLS_DATE_LANGUAGE=''AMERICAN'' NLS_SORT=''BINARY'' NLS_TIME_FORMAT=''HH.MI.SSXFF AM'' NLS_TIMESTAMP_FORMAT=''DD-MON-RR HH.MI.SSXFF AM'' NLS_TIME_TZ_FORMAT=''HH.MI.SSXFF AM TZR'' NLS_TIMESTAMP_TZ_FORMAT=''DD-MON-RR HH.MI.SSXFF AM TZR'' NLS_DUAL_CURRENCY=''$'' NLS_COMP=''BINARY'' NLS_LENGTH_SEMANTICS=''BYTE'' NLS_NCHAR_CONV_EXCP=''FALSE''');
dbms_scheduler.set_attribute('"JOB_FACTURACION_MASIVA_MD"','logging_level',DBMS_SCHEDULER.LOGGING_FULL);
dbms_scheduler.enable('"JOB_FACTURACION_MASIVA_MD"');
COMMIT; 
END; 
/ 
